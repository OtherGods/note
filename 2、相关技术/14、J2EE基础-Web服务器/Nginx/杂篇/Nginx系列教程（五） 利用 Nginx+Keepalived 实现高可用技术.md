...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182150341.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182154230.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182157970.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182159078.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182204716.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182205409.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182205468.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182206959.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182210222.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182211918.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182212989.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182216376.png)


**反向代理层到站点层的高可用**，通过站点层的冗余来实现，反向代理层是Nginx，Nginx.conf 里能够配置多个 Web 后端，并且 Nginx 能够检测多个后端的存活性。

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182217782.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182219083.png)


**站点层到服务层的高可用**，是通过服务层的冗余来实现的。“服务连接池”会建立与下游服务多个连接，每次请求会“随机”选取连接来访问下游服务。

![图片](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7962657a1cd5420fa54b26b75f373d61~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp "在这里插入图片描述")
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182223373.png)

**服务层到缓存层的高可用**，是通过缓存数据的冗余来实现，缓存层的数据冗余可通过利用客户端的封装，service 对 cache 进行双读或者双写方式。

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182224663.png)


缓存层也可以通过支持主从同步的缓存集群来解决缓存层的高可用问题，redis 天然支持主从同步，redis也有 sentinel 机制，来做 redis 的存活性检测。

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182224531.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182225787.png)

服务层到数据库读的高可用，是通过读库的冗余来实现，冗余了读库，一般来说就至少有2个从库，`数据库连接池`会建立与读库多个连接，每次请求会路由到这些读库里面去。

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182225214.png)

**故障自动转移**：当一台读库宕机时，db-connection-pool 能够检测到，会自动的进行故障转移，将流量自动迁移到其他的读库，整个过程由连接池自动完成，对调用方是透明的，数据库连接池是很重要的基础组件。

**服务层 --> 数据库层`写`的高可用**

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182226719.png)

**服务层到数据库写的高可用**，是通过写库的冗余来实现，可以设置两台`MySQL`双主同步，一台对线上提供服务，另一台做冗余以保证高可用，常见的实践是`keepalived`存活探测，相同`虚拟IP（virtual IP）`提供服务。

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182227957.png)

**故障自动转移**：当写库宕机时，`keepalived`能够检测到，会自动的进行故障转移，将流量自动迁移到`shadow-db-master`，使用的是相同的`虚拟IP（virtual IP）`，这个切换过程对调用方是透明的。

#### 九、配置高可用的准备工作

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182227913.png)

**_1、_** 准备两台 Nginx 服务器（IP 地址：192.168.1.10 和 192.168.1.11），并在两台 Nginx 服务器上安装`Keepalived`，以及配置`虚拟 IP 地址`；

**_2、_** 192.168.1.10 服务器，因为我们前期就已经安装好了 Nginx，无须在重新安装了，只需在 192.168.1.11 设备上安装 Nginx 服务即可，详细可以查看这篇文章：《[Nginx系列教程（一）| 手把手教你在Linux环境下搭建Nginx服务](https://juejin.cn/post/6968637428684816420 "https://juejin.cn/post/6968637428684816420")》；

**_3、_** 分别在两台`Nginx`服务器上安装`Keepalived`服务，可通过 rpm 包或 yum 一键安装，这两种安装方式都是可以的，根据个人所需安装即可；
...
**_4、_** 在两台`Nginx`服务器上分别启动`Nginx`服务和`keepalived`服务；
...
**_5、_** 在客户端浏览器中分别输入`192.168.1.10`和`192.168.1.11`进行验证是否能够正常访问`Nginx`服务；

![1.gif](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182233099.gif)
...
**_1、_** 主服务器上配置`keepalived.conf`配置文件
...
global_defs {  
  ...
  smtp_server 192.168.1.10               # 主服务器 IP 地址  
...
vrrp_script chk_http_port {  
   ...
   script "/usr/local/src/nginx_check.sh" # nginx_check.sh 脚本路径  
...
vrrp_instance VI_1 {  
    state MASTER           # 指定当前节点为 master 节点  
    interface eth0         # 这里的 eth0 是网卡的名称，通过 ifconfig 或者 ip addr 可以查看  
    virtual_router_id 51   # 这里指定的是虚拟路由 id，master 节点和 backup 节点需要指定一样的  
    priority 90            # 指定了当前节点的优先级，数值越大优先级越高，master 节点要高于 backup 节点  
...
**_2、_** 从服务器上配置`keepalived.conf`配置文件
...
vrrp_instance VI_1 {  
    state BACKUP           # 指定当前节点为 BACKUP 节点  
    interface eth1         # 这里的 eth0 是网卡的名称，通过 ifconfig 或者 ip addr 可以查看  
    virtual_router_id 51   # 这里指定的是虚拟路由 id，master 节点和 backup 节点需要指定一样的  
    priority 80            # 指定了当前节点的优先级，数值越大优先级越高，master 节点要高于 backup 节点  
    advert_int 1           # 指定发送VRRP通告的间隔，单位是秒  
    authentication {  
        auth_type PASS     # 鉴权，默认通过  
        auth_pass 1111     # 鉴权访问密码  
...
**_3、_** 将`nginx_check.sh`脚本分别放置在两台`Nginx`服务器的`/usr/local/src/`目录下，用于通过`Keepalived`来检测`Nginx主服务器`是否还活着，如果是已经宕机了，将自动切换至`从服务器`上面去。
...
**_4、_** 在两台`Nginx`服务器上分别配置`虚拟 IP 地址`，对客户端的响应是真实服务器直接返回给客户端的，而真实服务器需要将响应报文中的源 IP 地址修改为虚拟 IP 地址，这里配置的虚拟 IP 地址就是起这个作用的。
...
**_5、_** 在两台`Nginx`服务器上分别重启`Nginx`服务和`Keepalived`服务；
...
**_6、_**  在客户端浏览器中输入虚拟 IP 地址`192.168.1.100`测试访问结果；

![1.gif](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403182243425.gif)
...
[Nginx系列教程（一）| 手把手教你在Linux环境下搭建Nginx服务](https://juejin.cn/post/6968637428684816420 "https://juejin.cn/post/6968637428684816420")

[Nginx系列教程（二）| 一文带你读懂Nginx的正向与反向代理](https://juejin.cn/post/6968964548812374046 "https://juejin.cn/post/6968964548812374046")

[Nginx系列教程（三）| 一文带你读懂Nginx的负载均衡](https://juejin.cn/post/6969332977989845028 "https://juejin.cn/post/6969332977989845028")

[Nginx系列教程（四）| 一文带你读懂Nginx的动静分离](https://juejin.cn/post/6969704204285771807 "https://juejin.cn/post/6969704204285771807")

  

作者：杰哥的IT之旅  
链接：[https://juejin.cn/post/6970093569096810526](https://juejin.cn/post/6970093569096810526)  
来源：稀土掘金  
