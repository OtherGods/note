...
这篇文章会在 CentOS 环境下安装和使用 Nginx，如果对 CentOS 基本操作还不太清楚的，可以先看看 [半小时搞会 CentOS 入门必备基础知识](2、相关技术/11、Linux/掘金/基础知识/半小时搞会%20CentOS%20入门必备基础知识.md) 一文先做了解。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403252252061.png)
...
2. HTTP 头信息不超过右边这几个字段：`Accept`、`Accept-Language`、`Content-Language`、`Last-Event-ID``Content-Type` 只限于三个值 `application/x-www-form-urlencoded`、`multipart/form-data`、`text/plain`；
...
**==正向代理==：** 一般的访问流程是客户端直接向目标服务器发送请求并获取内容，**使用正向代理后，==客户端改为向代理服务器发送请求，并指定目标服务器（原始服务器）==，然后由代理服务器和原始服务器通信，转交请求并获得的内容，再返回给客户端**。<font color="blue">正向代理隐藏了真实的客户端</font>，为客户端收发请求，使<font color="blue">目标服务器不知道真实客户端，而真实客户端知道目标服务器；</font>

举个具体的例子 🌰，你的浏览器无法直接<font color="red" size=5>访问谷歌</font>，这时候可以通过一个代理服务器来帮助你访问谷哥，那么这个服务器就叫正向代理。

**==反向代理==：** 与一般访问流程相比，**使用反向代理后，==客户端向代理服务器发送请求，然后代理服务器将请求转发给内部网络上真正进行处理的服务器==，得到的结果返回给客户端**。<font color="blue">反向代理隐藏了真实的目标服务器</font>，为服务器收发请求，使<font color="blue">客户端不知道真实目标服务器</font>。一般在<font color="red" size=5>处理跨域请求</font>的时候比较常用。现在基本上所有的大型网站都设置了反向代理。

举个具体的例子 🌰，去饭店吃饭，可以点川菜、粤菜、江浙菜，饭店也分别有三个菜系的厨师 👨‍🍳，但是你作为顾客不用管哪个厨师给你做的菜，只用点菜即可，小二将你菜单中的菜分配给不同的厨师来具体处理，那么这个小二就是反向代理服务器。

简单的说，一般<font color="red" size=5><font color="blue" size=5>给客户端做代理</font>的都是正向代理，<font color="blue" size=5>给服务器做代理</font>的就是反向代理。</font>

正向代理和反向代理主要的原理区别可以参见下图：
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409211615810.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409211618524.png)


### 2.5 动静分离

为了加快网站的解析速度，可以把动态页面和静态页面由不同的服务器来解析，加快解析速度，降低原来单个服务器的压力。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409211619608.png)
...
来看看
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403252300425.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403252301487.png)

### 3.2 相关文件夹

然后我们可以使用 `rpm -ql nginx` 来查看 Nginx 被安装到了什么地方，有哪些相关目录，其中位于 `/etc` 目录下的主要是配置文件，还有一些文件见下图：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403252301171.png)
...
http {   # 配置使用最频繁的部分，代理、缓存、日志定义等绝大多数功能和第三方模块的配置都在这里设置
    ...
    server {
    	...
    	location / {
	    	# 定义了客户端请求的URI路径如何映射到服务器上的实际文件系统路径
...
#### 5.1.1、讯飞星火解释location

在nginx配置中，location指令用于匹配请求的URI，并根据匹配结果进行相应的处理。location可以有多种匹配方式，如精确匹配、前缀匹配、正则表达式匹配等。下面是location的详细用法及实际使用和匹配示例： 
1. 精确匹配（`=`）：当请求的URI与指定的路径完全相等时，匹配成功。
```bash
location = /example {
    # 配置信息
}
...
```bash
location = /example {
    return 200 'This is an exact match for /example';
}
```

访问`http://yourdomain.com/example`时，返回"This is an exact match for /example"。 

2. 前缀匹配（`^~`）：当请求的URI以指定的路径为前缀时，匹配成功。
```bash
location ^~ /example {
    # 配置信息
}
...
```bash
location ^~ /example {
    return 200 'This is a prefix match for /example';
}
```

访问`http://yourdomain.com/example/test`时，返回"This is a prefix match for /example"。 

3. 正则表达式匹配（`~、~*`）：当请求的URI满足指定的正则表达式时，匹配成功。其中，`~`表示区分大小写，`~*`表示不区分大小写。
```bash
...
```bash
location ~ /example[0-9]+ {
    return 200 'This is a regex match for /example followed by one or more digits';
}
```

访问`http://yourdomain.com/example123`时，返回"This is a regex match for /example followed by one or more digits"。

4. 通用匹配（`无修饰符`）：当请求的URI与指定的路径不完全相等，但仍然满足某种条件时，匹配成功。例如，可以匹配所有以/example开头的URI。
```bash
location /example {
    # 配置信息
}
...
```rust
location /example {
    return 200 'This is a generic match for any URI starting with /example';
}
```

访问`http://yourdomain.com/example/test`时，返回"This is a generic match for any URI starting with /example"。 注意：location的匹配顺序是：精确匹配 > 前缀匹配 > 正则表达式匹配 > 通用匹配。如果有多个匹配成功，将使用第一个匹配成功的location进行处理。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262126843.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262128200.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262128118.png)
...
1. `proxy_pass`：用于定义请求将被代理到的上游服务器；
2. `proxy_set_header`：在将客户端请求发送给后端服务器之前，更改来自客户端的请求头信息；
3. `proxy_connect_timeout`：配置 Nginx 与后端代理服务器尝试建立连接的超时时间；
4. `proxy_read_timeout`：配置 Nginx 向后端服务器组发出 read 请求后，等待响应的超时时间；
5. `proxy_send_timeout`：配置 Nginx 向后端服务器组发出 write 请求后，等待响应的超时时间；
6. `proxy_redirect`：用于修改后端服务器返回的响应头中的 Location 和 Refresh。
7. `proxy_cookie_domain`：用于修改从上游服务器（后端服务器）返回的 `Set-Cookie` 头部中的 `Domain` 属性

ChatGPT解释：

在 Nginx 配置中，以下指令和参数用于配置代理服务器相关的行为。它们帮助 Nginx 在反向代理的上下文中正确处理请求和响应。

### 7.1、代理相关指令和参数

1. **`proxy_pass`**
   - **含义**：`proxy_pass` 指令的主要作用是将匹配的请求转发给指定的上游服务器。这使得 Nginx 能够在前端处理请求，并将请求分发给适当的后端服务器进行处理。
   - **用法**：
```nginx
     proxy_pass http://backend_server;
```
   - **解释**：将匹配location后url的请求转发到`http://backend_server`;
2. **`proxy_set_header`**
   - **含义**：用于设置或修改传递给后端服务器的请求头。
   - **用法**：
     ```nginx
     ...
     ```
   - **解释**：
     - `Host $host`：将请求的 `Host` 头设置为客户端请求中的主机名。
     - `X-Real-IP $remote_addr`：将 `X-Real-IP` 头设置为客户端的真实 IP 地址。
     - `X-Forwarded-For $proxy_add_x_forwarded_for`：将 `X-Forwarded-For` 头设置为客户端的原始 IP 地址以及所有通过的代理服务器的 IP 地址。
3. **`proxy_connect_timeout`**
   - **含义**：设置 Nginx 和后端服务器之间建立连接的超时时间。
   - **用法**：
     ```nginx
     proxy_connect_timeout 60s;
     ```
   - **解释**：设置为 60 秒，如果在该时间内无法建立连接，则认为连接超时。
4. **`proxy_read_timeout`**
   - **含义**：设置 Nginx 从后端服务器读取响应的超时时间。
   - **用法**：
     ```nginx
     proxy_read_timeout 60s;
     ```
   - **解释**：设置为 60 秒，如果在该时间内没有从后端服务器读取到数据，则认为读取超时。
5. **`proxy_send_timeout`**
   - **含义**：设置 Nginx 向后端服务器发送请求的超时时间。
   - **用法**：
     ```nginx
     proxy_send_timeout 60s;
     ```
   - **解释**：设置为 60 秒，如果在该时间内没有成功将请求发送给后端服务器，则认为发送超时。
6. **`proxy_redirect`**
   - **含义**：用于修改后端服务器返回的重定向响应中的 `Location` 和 `Refresh` 头。
   - **用法**：
     ```nginx
     ...
     ```
   - **解释**：
     - `proxy_redirect http://backend.example.com/ http://frontend.example.com/;`：将后端服务器返回的 `Location` 头中的 `http://backend.example.com/` 替换为 `http://frontend.example.com/`。
     - `proxy_redirect default;`：使用默认的重定向行为。
7. **`proxy_cookie_domain`**
   - **含义**：用于修改从上游服务器（后端服务器）返回的 `Set-Cookie` 头部中的 `Domain` 属性。这在反向代理场景下非常有用，因为你可能希望将后端服务器设置的 cookie 域修改为客户端访问的域。
   - **用法**：
```nginx
	 proxy_cookie_domain backend.example.com frontend.example.com;
```
   - **解释**：当 Nginx 作为反向代理服务器时，上游服务器可能会设置 `Set-Cookie` 头部中的 `Domain` 属性。例如，上游服务器设置的 `Domain` 属性可能是 `backend.example.com`，但客户端实际访问的是 `frontend.example.com`。为了确保 cookie 能够正确地被客户端接受并发送回服务器，可能需要修改 `Set-Cookie` 头部中的 `Domain` 属性。这就是 `proxy_cookie_domain` 的作用
...
- **`proxy_set_header`**：设置或修改传递给后端服务器的请求头。
- **`proxy_connect_timeout`**：设置与后端服务器建立连接的超时时间。
- **`proxy_read_timeout`**：设置从后端服务器读取响应的超时时间。
- **`proxy_send_timeout`**：设置向后端服务器发送请求的超时时间。
- **`proxy_redirect`**：修改后端服务器返回的重定向响应中的 `Location` 和 `Refresh` 头。

这些配置指令在反向代理场景下非常有用，可以帮助 Nginx 更好地处理与后端服务器的交互和响应。

## 8. 跨域 CORS 配置

关于简单请求、非简单请求、跨域的概念，前面已经介绍过了，还不了解的可以看看前面的讲解。现在前后端分离的项目一统天下，经常本地起了前端服务，需要访问不同的后端地址，不可避免遇到跨域问题。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262206637.png)
...
这样就将对前一个域名 `fe.sherlocked93.club` 的请求全都代理到了 `be.sherlocked93.club`，前端的请求都被我们用服务器代理到了后端地址下，绕过了跨域(不是绕过了跨域，是绕过了浏览器的同源策略)。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262222425.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262224493.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262225806.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262230983.png)


配置之后 response 的 header 里面多了一个 `Content-Encoding: gzip`，返回信息被压缩了：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262230798.png)


注意了，一般 gzip 的配置建议加上 `gzip_min_length 1k`，不加的话：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262231286.png)
...
由此打包出来的文件如下图：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262231016.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262238701.png)
...
可以看到在模拟使用移动端访问的时候，Nginx 返回的站点变成了移动端对应的 html 了。![1.gif](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403262241671.gif)
...
> 参考文档：
> - [Nginx中文文档](https://link.juejin.cn?target=https%3A%2F%2Fwww.nginx.cn%2Fdoc%2F "https://www.nginx.cn/doc/")
> - [Nginx安装，目录结构与配置文件详解](https://link.juejin.cn?target=https%3A%2F%2Fwww.cnblogs.com%2Fliang-io%2Fp%2F9340335.html "https://www.cnblogs.com/liang-io/p/9340335.html")
> - [Keepalived安装与配置](https://link.juejin.cn?target=https%3A%2F%2Fblog.csdn.net%2Fxyang81%2Farticle%2Fdetails%2F52554398 "https://blog.csdn.net/xyang81/article/details/52554398")
> - [Keepalived+Nginx实现高可用](https://link.juejin.cn?target=https%3A%2F%2Fblog.csdn.net%2Fxyang81%2Farticle%2Fdetails%2F52556886 "https://blog.csdn.net/xyang81/article/details/52556886")
