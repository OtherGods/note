...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403102144562.png)

## 3.2、反向代理

<font color = "red" size=5>客户端不知道是那台服务器处理的请求</font>
<font color = "blue" size=4>实例：负载均衡</font>

客户端将请求发送到反向代理服务器，由反向代理服务器去选择目标服务器，获取数据后再返回给客户端。对外暴露的是反向代理服务器地址，隐藏了真实服务器 IP 地址。反向代理“代理”的是目标服务器，这一个过程对于客户端而言是透明的。

举个例子：公司内网部署了 3 台服务器，客户端请求直接经过代理服务器，由代理服务器将请求转发到内网服务器并最终决定哪一台服务器处理客户端请求。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403102147607.png)
...
<font color = "red" size=5>服务端不知道是那台客户端发送的请求</font>
<font color = "blue" size=4>实例：科学上网</font>
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403102154131.png)
...
相关阅读：
- [使用NGINX作为HTTPS正向代理服务器](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/使用NGINX作为HTTPS正向代理服务器.md)
- [图解及代码实现正向代理、反向代理及负载均衡](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/图解及代码实现正向代理、反向代理及负载均衡.md)

## 3.4、负载均衡

如果一台服务器处理用户请求处理不过来的话，一个简单的办法就是增加多台服务器（服务器集群）部署相同的服务来处理用户请求。

Nginx 可以将接收到的客户端请求以一定的规则（负载均衡策略） **均匀地分配** 到这个服务器集群中所有的服务器上，这个就叫做 负载均衡。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403102213544.png)
...
> 相关参考：
>  
> 	1. [五分钟看懂 Nginx 负载均衡](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/五分钟看懂%20Nginx%20负载均衡.md)
> 	2. [Nginx 官方文档](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer/)
>  
> Nginx 的负载均衡策略不止下面介绍的这四种，我这里只是列举几个比较常用的负载均衡策略。

## 5.1、轮询（Round Robin，默认）
...
- 开启 Gzip 压缩 ：这样可以使网站的图片、CSS、JS 等文件在传输时进行压缩，提高访问速度, 优化 Nginx 性能。详细介绍可以参考 [Nginx性能优化功能- Gzip压缩(大幅度提高页面加载速度)](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/Nginx性能优化功能-%20Gzip压缩(大幅度提高页面加载速度).md) 这篇文章；
...
我们可以利用第三方模块 **upstream_check_module** 来检测后端服务的健康状态，如果后端服务器不可以用，则所有的请求不转发到这台服务器

**upstream_check_module** 是以款阿里大佬开源的，使用Perl 和 C编写而成的，Github地址：[https://github.com/yaoweibin/nginx_upstream_check_module](https://github.com/yaoweibin/nginx_upstream_check_module)

关于 **upstream_check_module** 实现后端服务健康检查的具体做法可以参考 [Nginx负载均衡健康检查功能](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/Nginx负载均衡健康检查功能.md) 这篇文章。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403112101445.png)
...
Keepalived 不仅仅可以和 Nginx 搭配使用，还可以和 [LVS、MySQL、HAProxy](https://www.yuque.com/r/goto?url=https%3A%2F%2Fprogrammer.group%2Fhigh-availability-scheme-implementation-of-mysql-master-replication-keepalived.html) 等软件配合使用。

再来简单介绍一下Keeplived + Nginx实现高可用的常见方法：
- 准备两台Nginx服务器，一台为主服务器，另一台为备用服务器
- 在两台Nginx服务器上安装并配置Keepalived
- 为两台Nginx服务器绑定同一个虚拟IP
- 编写Nginx检测脚本用于通过Keepalived检测Nginx主服务器的状态是否正常

如果Nginx主服务器宕机的话，会自动进行故障转移，备用Nginx主服务器升级为主服务器。并且，这个切换对外是透明的，因为使用的虚拟IP，虚拟IP不会改变。

相关阅读：
- [Nginx系列教程（五） 利用 Nginx+Keepalived 实现高可用技术](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/Nginx系列教程（五）%20利用%20Nginx+Keepalived%20实现高可用技术.md)
- [搭建Keepalived Nginx高可用Web集群](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/搭建Keepalived%20Nginx高可用Web集群.md)

> 📄友情提示 ：下面的内容属于 Nginx 的进阶指点，主要是一些 Nginx 底层原理相关的知识。你可以根据自身情况选择是否掌握这部分内容，如果你的简历没有写熟练掌握 Nginx 使用及原理的话，面试官一般不会问这么深入。

# 11、Nginx总体架构了解吗？

> 关于 Nginx 总结架构的详细解答，请看这篇文章：[最近和Nginx杠上了！](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/最近和Nginx杠上了！.md)

对于传统的 HTTP 和反向代理服务器而言，在处理并发请求的时候会使用单进程或线程的模式处理，同时会阻止网络或输入/输出操作。
...
一张图了解Nginx的总结架构：
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403112122685.png)

# 12、Nginx进程模型了解吗？

> 关于进程模型的详细解答，请看这篇文章：[Nginx 工作模式和进程模型](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/Nginx%20工作模式和进程模型.md)
...
- [Nginx 是如何处理 HTTP 头部的](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/Nginx%20是如何处理%20HTTP%20头部的.md)
- [一次性弄懂 Nginx 处理 HTTP 请求的 11 个阶段](2、相关技术/14、J2EE基础-Web服务器/Nginx/杂篇/一次性弄懂%20Nginx%20处理%20HTTP%20请求的%2011%20个阶段.md)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403112130462.png)

你可以配合 [nginx-tutorial](https://github.com/dunwu/nginx-tutorial) 这个开源 Nginx 极简教程食用。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403112131910.png)

最后再推荐一个可以在线练习使用 Nginx 的网站：[https://nginx-playground.wizardzines.com/](https://nginx-playground.wizardzines.com/) 。

![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403112132548.png)
