...
| 索引（Index）           | 索引（Index）                |     |
| 嵌套表（Embedded Table） | 嵌入式文档（Embedded Dcoument） |     |
| 数组（Array）           | 数组（Array）                |     |
#### 1.2.1、文档

MongoDB 中的记录就是一个 BSON 文档，它是由键值对组成的数据结构，类似于 JSON 对象，是 MongoDB 中的基本数据单元。字段的值可能包括其他文档、数组和文档数组。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202511161935984.png)
...
根据维基百科对 BSON 的介绍，BSON 的遍历速度优于 JSON，这也是 MongoDB 选择 BSON 的主要原因，但 BSON 需要更多的存储空间。

> 与 JSON 相比，BSON 着眼于提高存储和扫描效率。BSON 文档中的大型元素以长度字段为前缀以便于扫描。在某些情况下，由于长度前缀和显式数组索引的存在，BSON 使用的空间会多于 JSON。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202511161938172.png)

#### 1.2.2、集合

MongoDB 集合存在于数据库中，**没有固定的结构**，也就是 **无模式** 的，这意味着可以往集合插入不同格式和类型的数据。不过，通常情况下，插入集合中的数据都会有一定的关联性。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202511161940673.png)
...
- 是否需要**大数据量的存储**？是否需要**快速水平扩展**？MongoDB 支持分片集群，可以很方便地添加更多的节点（实例），让集群存储更多的数据，具备更强的性能。
- 是否需要**更多类型索引**来满足更多应用场景？MongoDB 支持多种类型的索引，包括单字段索引、复合索引、多键索引、哈希索引、文本索引、 地理位置索引等，每种类型的索引有不同的使用场合。
- ……
...
其整体结构如下图所示：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202511162220826.png)
...
2. 对这些文档进行一系列运算
3. 结果文档输出给下一个阶段
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202511162222418.png)
...
与关系型数据库一样，MongoDB 4.x 事务同样具有 ACID 特性：
...
关于事务的详细介绍这篇文章就不多说了，感兴趣的可以看看我写的[MySQL 常见面试题总结](/database/mysql/mysql-questions-01.html)这篇文章，里面有详细介绍到。
...
- 《MongoDB 权威指南》
- 技术干货| MongoDB 事务原理 - MongoDB 中文社区：[https://mongoing.com/archives/82187](https://mongoing.com/archives/82187)
- Transactions - MongoDB 官方文档：[https://www.mongodb.com/docs/manual/core/transactions/](https://www.mongodb.com/docs/manual/core/transactions/)
- WiredTiger Storage Engine - MongoDB 官方文档：[https://www.mongodb.com/docs/manual/core/wiredtiger/](https://www.mongodb.com/docs/manual/core/wiredtiger/)
- WiredTiger 存储引擎之一：基础数据结构分析：[https://mongoing.com/topic/archives-35143](https://mongoing.com/topic/archives-35143)

著作权归JavaGuide(javaguide.cn)所有 基于MIT协议 原文链接：https://javaguide.cn/database/mongodb/mongodb-questions-01.html
