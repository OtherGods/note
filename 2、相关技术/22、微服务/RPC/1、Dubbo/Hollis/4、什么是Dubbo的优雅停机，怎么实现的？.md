...
Dubbo是通过 **事件 + JDK的shutdown hook** 来完成优雅停机（下线）的。
...
在操作系统、容器等对优雅上下线有了基本的支持之后，在接收到docker stop、`kill -15`等命令后，会通知应用进程进行进程关闭。

而Java应用在运行时就是一个独立运行的进程，这个进程是如何关闭的呢？

Java程序的终止运行是基于JVM的关闭实现的，JVM关闭方式分为正常关闭、强制关闭和异常关闭3种:
> JVM关闭方式分为3种：
> 1. **正常关闭**：当最后一个非守护线程结束或者调用了System.exit或者通过其他特定平台的方法关闭（接收到SIGINT（2）、SIGTERM（15）信号等）、docker stop
> 2. **强制关闭**：通过调用Runtime.halt方法或者是在操作系统中强制kill（接收到SIGKILL（9）信号)、docker kill
> 3. **异常关闭**：运行中遇到RuntimeException异常等。
> 参考：[Java是如何处理SIGTERM(15)的](4、对JDK进程执行kill%20-9有什么影响？#Java是如何处理SIGTERM(15)的)

这其中，<font color="red" size=5>正常关闭就是支持优雅上下线的</font>。正常关闭过程中，JVM可以做一些清理动作，比如删除临时文件。
...
可以看到，当我们使用`kill（默认kill -15）`关闭进程的时候，程序会先执行我注册的shutdownHook，然后再退出，并且会给出一个提示：interrupted by signal 15: SIGTERM
