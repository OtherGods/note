...
电商网站检索：
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052202830.png)

ELK 日志采集系统架构（负责日志的搜索）：
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052204059.png)

## 1.4、为什么需要Elasticsearch？MySQL不行吗？
...
## 1.5、Elasticsearch中的基本概念

- **Index（索引）** ： 作为名词理解的话，索引是一类拥有相似特征的文档的集合比如商品索引、商家索引、订单索引，有点类似于 MySQL 中的 **Table（表）**。作为动词理解的话，索引就是将一份文档保存在一个索引中。
- **Mapping（映射）** ：定义字段名称、数据类型、优化信息（比如是否索引)、分词器，有点类似于数据库中的**表结构定义（Schema）**。6.x 及更早版本中，一个 Index 对应多个 Mapping。7.x 开始，一个 Index 对应一个 Mapping。
...
**倒排索引** 也被称作反向索引（inverted index），是用于提高数据检索速度的一种数据结构，空间消耗比较大。倒排索引首先将检索文档进行分词得到多个词语/词条，然后将<font color = "red" size = 5>词语和包含该词语的文档 ID 建立关联</font>，从而提高检索效率。<font color="red"size=5 >【我的理解是：正排索引是知道位置（比如：ID）找内容，倒排索引是知道内容找位置】</font>
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052231045.png)

- **文档（Document）** ：用来被搜索的数据，其中的每一条数据就是一个文档。例如一个商品信息、商家信息、一页网页的内容。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052238761.png)
...
不同于倒排索引，正排索引将<font color = "red" size = 5>文档 ID 和分词建立关联</font>。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052243254.png)
...
## 2.6、Elasticsearch可以针对某些字段不做索引吗？

在构建mapping时，可以指定index为false（默认为true），就可以对该字段不建立倒排索引
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052256249.png)
...
**中文分词器：**
- **IK Analyzer（推荐）**： 最常用的开源中文分词器，Github 地址：[https://github.com/medcl/elasticsearch-analysis-ik](https://github.com/medcl/elasticsearch-analysis-ik)，包括两种分词模式：
...
- 分词结果：
	- 细粒度切分模式：`[数据库,数据,索引,可以,大幅,提高,查询,速度]`
	- 智能模式：`[数据库,索引,可以,大幅,提高,查询,速度]`
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052307620.png)

## 3.4、Elasticsearch如何基于拼音搜索？
...
在 Elasticsearch 中，数据类型主要指的是文档中的字段类型，而不是索引的类型。每个字段都需要指定其数据类型，这决定了 Elasticsearch 如何索引和查询这些字段的数据。

在创建索引时，可以通过映射（mappings）定义字段的数据类型。例如：
```json
PUT /my_index
{
  "mappings": {
...
代码解释：
- **`name` 字段**：
    - 类型是 `text`，适合用于存储需要全文检索的文本数据。
- **`tags` 字段**：
    - 类型是 `keyword`，适合用于存储标签、分类等不需要分词的数据。
- **`age` 字段**：
    - 类型是 `integer`，适合用于存储整数值，如年龄。
- **`height` 字段**：
    - 类型是 `float`，适合用于存储浮点数值，如身高。
- **`birthdate` 字段**：
    - 类型是 `date`，指定了日期格式 `yyyy-MM-dd`。
- **`active` 字段**：
    - 类型是 `boolean`，适合用于存储布尔值。
- **`location` 字段**：
    - 类型是 `geo_point`，适合用于存储地理位置信息。
- **`address` 字段**：
    - 类型是 `object`，包含子字段 `street` 和 `city`。
- **`comments` 字段**：
    - 类型是 `nested`，适合存储嵌套对象数组，包含子字段 `author`、`message` 和 `timestamp`。

## 4.1、Elasticsearch常见的数据类型有那些？
...
## 4.2、keyword 和 text 有什么区别？

keyword 不走分词器，而 text 会走分词器，使用 keyword 关键字查询效率更高，一般在 fields 中定义keyword类型字段
```json
"name" : {
          "type" : "text",
          "fields" : {
...
在 Elasticsearch 中，`keyword` 和 `text` 是两种不同的数据类型，用于不同的场景。下面是它们的主要区别，以及在代码示例中的应用：

**`text` 类型**

- **用途**：*用于全文搜索。文本会被分词（tokenized），然后每个词被索引，用于搜索和分析。*
- **特点**：
  - 支持全文搜索。
  - 适合包含多个词的长文本字段，如文章内容、评论、描述等。
  - 分词器会将文本拆分为独立的词语，支持复杂的查询如 `match` 查询。

**`keyword` 类型**

- **用途**：*用于精确匹配。文本不会被分词，而是作为一个整体被索引。*
- **特点**：
  - 支持精确匹配查询。
  - 适合标识符、标签、邮箱地址、邮政编码等需要精确匹配的字段。
  - 不进行分词，索引速度较快。

**代码示例解释**

假设有一个包含用户信息的文档，其中有一个 `name` 字段。我们希望能够对 `name` 字段进行全文搜索（例如，搜索包含某些词的名字），也希望能够对 `name` 字段进行精确匹配（例如，查找特定的名字）。
...
在这个示例中，`name` 字段被定义为 `text` 类型，并且有一个子字段 `keyword`，定义为 `keyword` 类型。这种定义方式允许同一个字段既可以进行全文搜索，也可以进行精确匹配。

- **`name` (text 类型)**：
  - 当使用 `name` 字段进行查询时，Elasticsearch 会对文本进行分词，可以执行全文搜索。例如，查询 `{"match": {"name": "John Doe"}}` 将搜索包含 "John" 或 "Doe" 的文档。
  
- **`name.keyword` (keyword 类型)**：
  - 当使用 `name.keyword` 字段进行查询时，Elasticsearch 会对文本进行精确匹配。例如，查询 `{"term": {"name.keyword": "John Doe"}}` 只会搜索完全匹配 "John Doe" 的文档。
...
```
- 返回结果包含名字中有 "John" 的文档，如 "John Doe" 和 "John Smith"。

**精确匹配 (`keyword` 类型)**：查找名字为 "John Doe" 的文档
```json
GET /my_index/_search
{
  "query": {
...
```
- 返回结果只有名字完全匹配 "John Doe" 的文档。

**解释**
- **`text` 类型字段**（`name`）会被分词，例如 "John Doe" 会被分成 "John" 和 "Doe" 两个词，因此可以通过 `match` 查询找到包含这些词的文档。
- **`keyword` 类型字段**（`name.keyword`）不会被分词，作为一个整体索引，因此只有完全匹配 "John Doe" 的文档会被找到。

**具体应用场景**

- **全文搜索**：用户搜索产品描述、文章内容等，可以使用 `text` 类型字段，这样可以找到包含搜索词的所有文档。
- **精确匹配**：用户搜索特定的标签、分类、用户名等，可以使用 `keyword` 类型字段，以确保只返回完全匹配的结果。

这种组合使用方式可以提供更灵活的查询能力，让 Elasticsearch 同时支持全文搜索和精确匹配。

## 4.3、Elasticsearch 是否有数组类型？

在 Elasticsearch 中，没有专门的数组数据类型。默认情况下，任何字段都可以包含零个或多个值，但是，数组中的所有值必须具有相同的数据类型。

Elasticsearch 怎么修改索引字段类型？

## 4.4、可以在 Mapping 中直接修改字段类型吗？

不可以！Elasticsearch 中的 Mapping 有点类似于数据库中的表结构定义，Mapping 中的字段类型只能增加不能修改，否则只能 reindex 重新索引或者重新进行数据建模并导入数据。

## 4.5、什么是 Nested 数据类型？有什么用？
...
可以来试着搜索看下结果，如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052357034.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052357974.png)

再新建my-index索引，不过这次先定义好user对象的类型是nested，如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052358298.png)

然后再次新增数据，如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052359419.png)

再次重复之前的搜索，结果如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409052359525.png)

结果为空，符合预期，因为数据中确实没有名为Alice Smith的人。

## 4.6、将多个字段值合并为一个字段怎么做？
...
### 4.6.1、使用 `copy_to` 的示例

假设我们有一个索引 `my_index`，其中包含 `first_name` 和 `last_name` 字段，我们希望创建一个名为 `full_name` 的字段，用于包含这两个字段的合并内容。

### 4.6.2、步骤

1. **创建索引并定义映射**： 在创建索引时，定义 `first_name` 和 `last_name` 字段，并使用 `copy_to` 将它们的内容复制到 `full_name` 字段。
```json
PUT /my_index
{
  "mappings": {
...
```
2. **添加文档**： 当我们添加一个包含 `first_name` 和 `last_name` 字段的文档时，这些字段的内容会自动复制到 `full_name` 字段。
```json
POST /my_index/_doc/1
{
  "first_name": "John",
...
```
3. **查询合并后的字段**： 现在我们可以使用 `full_name` 字段来进行搜索，它包含了 `first_name` 和 `last_name` 的内容。
```json
GET /my_index/_search
{
  "query": {
...
1. **定义映射**：
    - 在索引的 `mappings` 中，我们定义了 `first_name` 和 `last_name` 字段的类型为 `text`，并且都使用了 `copy_to` 参数，将它们的内容复制到 `full_name` 字段。
    - `full_name` 字段也被定义为 `text` 类型，用于存储合并后的内容。
2. **添加文档**：
    - 当我们添加一个文档时，只需提供 `first_name` 和 `last_name` 字段。Elasticsearch 会自动将这两个字段的内容复制到 `full_name` 字段中。
3. **查询**：
    - 通过查询 `full_name` 字段，我们可以搜索到包含 `John` 和 `Doe` 的文档，因为这些内容已经被复制到了 `full_name` 字段中。

### 4.6.4、使用 `copy_to` 的注意事项

- `copy_to` 不能用来在同一个文档中复制数据到现有字段。这意味着 `full_name` 字段必须是一个新的字段，而不是现有的 `first_name` 或 `last_name` 字段。
- `copy_to` 只是在索引过程中起作用。查询时，`full_name` 字段实际上并不存在于源文档中，因此在 `_source` 中查看时不会显示 `full_name` 字段的内容。

### 4.6.5、结论

`copy_to` 是一个强大的功能，可以将多个字段的内容合并到一个字段中，以便进行统一搜索。通过正确定义映射并使用 `copy_to`，可以简化复杂的搜索需求，提高搜索的灵活性和效率。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409081058654.png)

## 5.5、动态 Mapping 如何防止字段无限增加？

> 摘自官方文档：Mapping limit settings 。

如果使用了动态映射，插入的每个新文档都可能引入新字段。在索引中定义太多字段会导致 映射爆炸 ，从而导致内存不足的错误和难以恢复的情况。使用 映射限制设置 在创建索引时来限制字段映射的数量（手动或动态创建）并防止映射爆炸。
例如：
```json
...
- index.mapping.nested_fields.limit：nested索引中不同映射的最大数量，nested类型只应在需要相互独立地查询对象数组时使用，默认为 50。
- index.mapping.nested_objects.limit：单个文档可以包含的嵌套 JSON 对象（nested类型）的最大数量，默认为 10000。
...
- docs: 倒排索引中，只包括 docID
- freqs: 倒排索引中，包括 docID/词频
- options：倒排索引中，默认属性，docID/词频/位置
- offsets: 倒排索引中，docID/词频/位置/字符偏移量
...
- 叶子查询：指定条件指定字段查询，包括term分词查询和全文检索（match，match_phrase)
- 复合查询：可包含叶子查询语句和复合查询，主要包括bool和 dis_max
...
## 6.4、Match 和 Match_phrase 区别？

match 查询多个检索词之间默认是 or 关系，可使用 operator 改为 and 关系

match_phrase 查询多个检索词之间默认是 and 关系，并且词的位置关系影响搜索结果
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409081149800.png)


## 6.5、Multi match 有几种匹配策略，都有什么区别？
...
- cross_fields ：跨字段匹配,解决了most_fields 查询词无法使用 and 连接的问题，匹配更加精确，and相当于整合多个字段为一个字段，但又不像 copy_to 占用存储空间。
...
### 6.5.1、multi_match 与 match对比


在 Elasticsearch 中，`multi_match` 和 `match` 查询都是用来进行全文搜索的，但它们有一些关键的区别。下面我将详细解释它们之间的差异，并给出实际示例。

1. **`match` 查询**

**描述**：`match` 查询在单个字段上进行全文搜索。
...
**解释**：这个查询会在 `title` 字段中搜索包含 "Elasticsearch guide" 的文档。`match` 查询会分析查询字符串并分词，然后在指定字段中寻找这些词的匹配项。

**`multi_match` 查询**

**描述**：`multi_match` 查询在多个字段上进行全文搜索。它是 `match` 查询的多字段扩展。
...
**解释**：这个查询会在 `title`、`description` 和 `content` 字段中搜索包含 "Elasticsearch guide" 的文档。`multi_match` 查询允许在多个字段中进行全文搜索，适用于需要在多个相关字段中查找信息的场景。

**相同**
1. 都是或的关系，上面两个查询的结果中都有title字段中包含"Elasticsearch"或"guide"的数据

**区别**

1. **单字段 vs 多字段**：
   - `match` 查询仅在一个字段上搜索。
   - `multi_match` 查询在多个字段上搜索。

2. **用途**：
   - `match` 查询用于在特定字段中查找匹配项，适用于简单的搜索需求。
   - `multi_match` 查询用于在多个字段中查找匹配项，适用于需要在多个相关字段中查找信息的场景。

3. **查询类型**：
   - `match` 查询的查询类型通常为 `match` 或 `match_phrase`。
   - `multi_match` 查询支持多种类型，如 `best_fields`、`most_fields`、`cross_fields`、`phrase`、`phrase_prefix` 和 `bool_prefix`。
...
```json
{
  ...
  "content": "This guide covers everything you need to know about Elasticsearch."
}
```
...
- 这个查询仅在 `title` 字段中搜索包含 "Elasticsearch guide" 的文档。
...
- 这个查询在 `title`、`description` 和 `content` 字段中搜索包含 "Elasticsearch guide" 的文档。

**结果**

- 使用 `match` 查询，只有 `title` 字段中包含 "Elasticsearch"或"guide" 的文档会被返回。
- 使用 `multi_match` 查询，只要 `title`、`description` 或 `content` 字段中包含 "Elasticsearch"或"guide" 的文档都会被返回。

**总结**

`match` 查询和 `multi_match` 查询在功能上有显著差异。`match` 查询用于单个字段的全文搜索，而 `multi_match` 查询用于多个字段的全文搜索。选择哪种查询取决于具体的搜索需求，如果需要在多个字段中查找信息，`multi_match` 查询是更合适的选择。
...
- <font color="red" size=5>当你初始化一个 scroll 请求时，Elasticsearch 会创建一个当前搜索结果的快照。这个快照会保存所有匹配文档的状态，即使之后这些文档被更新或删除，scroll 返回的结果也不会改变。实时分页场景中，用户需要查看最新的数据。</font>

### **3、sort + search_after分页**
...
# 7、数据同步
## 7.1、Elasticsearch 和 MySQL 同步的策略有哪些？
...
另外，除了插件之外，像我们比较熟悉的 Canal 除了支持 binlog 实时增量同步数据库之外也支持全量同步 。
...
## 7.2、Canal 增量数据同步 Elasticsearch 的原理了解吗？
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409081628291.png)

# 8、Elasticsearch 集群
## 8.1、Elasticsearch 集群是什么？有什么用？
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409081630516.png)

Elasticsearch 集群可以很方便地实现横向扩展，我们可以动态添加或者删除 Elasticsearch 节点。当有节点加入集群中或者从集群中移除节点时，集群将会重新平均分布所有的数据。

## 8.2、Elasticsearch 集群中的节点角色有哪些？
...
routing 是一个可以配置的变量,默认是使用文档的 id。对 routing取哈希再除以number_of_primary_shards(索引创建时指定的分片总数)得到的余数就是对应的分片。
...
- [lasticSearch 文档分值 score 计算&聚合搜索案例分析 - 政采云技术团队 - 2022](https://juejin.cn/post/7134855425714815012)
...
转载自：[Elasticsearch 常见面试题总结](https://www.yuque.com/snailclimb/mf2z3k/simu3iyhd87kgp3e)
