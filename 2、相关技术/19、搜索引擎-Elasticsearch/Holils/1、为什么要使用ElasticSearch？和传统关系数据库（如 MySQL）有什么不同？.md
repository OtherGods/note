...
**Elasticsearch 是基于文档的==分布式搜索引擎==，它==使用 JSON 文档来存储数据==**。在 Elasticsearch 中，相关的数据通常存储在同一个文档中，而不是分散在多个表中。

**MySQL 是一个==关系型数据库管理系统==，它==使用表、行和列的结构来组织数据==**。数据通过外键关系分散在多个表中。


## MySQL与ElasticSearch概念比对
...
**Elasticsearch 不支持传统的 ACID（原子性、一致性、隔离性、持久性）事务**。虽然*它确保了单个文档操作的原子性，但不适用于跨多个文档的复杂事务*。

**MySQL 支持 ACID 事务**，这使得它非常适合需要严格数据一致性的应用，如金融服务和其他商业数据处理。

## ElasticSearch主要场景和优势

Elasticsearch是一个开源的分布式搜索和分析引擎，主要适用于以下场景：

1. **搜索引擎**：用于 **==快速检索数据==**，比如文档、商品、新闻等。
