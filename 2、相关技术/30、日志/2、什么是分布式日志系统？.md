...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508251556025.png)

一个好的分布式日志系统，应该具备数据采集、数据加工、查询分析、监控报警、日志审计等功能。有了分布式日志系统，我们就可以做集中化的日志管理，（准）实时性的做日志查询及分析，快速的做问题排查，更好的做数据分析及挖掘。

比较主流的这类日志管理系统有`ELK`、`Graylog`、`Apache Flume`，还有很多类似的云产品，如阿里云的`SLS`。

一般来说，**如果资金够就上`SLS`，不够就自建`ELK`**。
...
`Elasticsearch`是个开源分布式搜索引擎，提供分析、存储数据等功能。
...
所以，**通常是使用`Logstash`做日志的采集与过滤，`ES`做分析和查询，`Kibana`做图形化界面**。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508251603551.png)
