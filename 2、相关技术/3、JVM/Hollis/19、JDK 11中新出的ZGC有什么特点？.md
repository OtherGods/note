...
**ZGC（Z Garbage Collector）** 是Java 11中引入的一种新的垃圾回收器，他是一个**为了实现低延迟而设计的垃圾收集器**，具有以下几个特点：
...
因此，ZGC是一种新的、高效的、低停顿的垃圾回收器，适用于内存大小从几GB到数TB的应用程序。它的设计目标是<font color="red" size=5>在保证高吞吐量的同时保证最短的暂停时间</font>，并且易于使用和维护。
