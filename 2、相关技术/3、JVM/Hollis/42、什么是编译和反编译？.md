...
编程语言（Programming Language）分为低级语言（Low-level Language）和高级语言（High-level Language）。**低级语言是计算机认识的语言、高级语言是程序员认识的语言。**
- **机器语言**（Machine Language）和**汇编语言**（Assembly Language）属于低级语言，直接用计算机指令编写程序。
...
参考：[1、Java是如何实现的平台无关？](2、相关技术/3、JVM/Hollis/1、Java是如何实现的平台无关？.md)

在编译原理中，把源代码翻译成机器指令，一般要经过以下几个重要步骤：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508021709124.png)

我们可以把将`.java`文件编译成`.class`的编译过程称之为前端编译。把将.class文件翻译成机器指令的编译过程称之为后端编译。
...
介绍3个Java的反编译工具：`javap`、`jad`和`cfr`

### javap

[javap命令的作用是什么？](2、相关技术/3、JVM/Hollis/javap命令的作用是什么？.md)
...
所以`--decodestringswitch`表示对于`switch`支持`string`的细节进行解码。类似的还有`--decodeenumswitch`、`--decodefinally`、`--decodelambdas`等。在我的关于语法糖的文章中，我使用`--decodelambdas`对lambda表达式警进行了反编译。 源码：
```java
public static void main(String... args) {
...
- 隔离Java程序
	- 让用户接触不到你的Class文件
- 对Class文件进行**加密**，在自定义的类加载器中指定解密方式进行解密，示例代码，参照：[双亲委托的作用](24、什么是双亲委派？如何破坏？#双亲委托的作用)
	- 提到破解难度
- **代码混淆**
