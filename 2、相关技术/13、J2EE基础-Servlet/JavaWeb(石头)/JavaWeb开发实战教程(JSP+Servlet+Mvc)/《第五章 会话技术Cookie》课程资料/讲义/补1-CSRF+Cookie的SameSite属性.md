...
跨域补充来自掘金：[我知道的跨域与安全](https://juejin.cn/post/6844903553069219853)

CSRF补充：[前端安全系列之二：如何防止CSRF攻击？](https://juejin.cn/post/6844903689702866952)
这篇文章介绍的比较好，但是有很多内容我用不到，所以只是在我的笔记中的几个地方贴了一些，但是我又怕这篇文章之后不能看了，所以直接把原文贴下来了，对应的笔记是：[补5-前端安全系列之二：如何防止CSRF攻击？.md](D:\c_51CTO资料\Java\石头老师Java工程师养成之路（部分）\JavaWeb开发实战教程(JSP+Servlet+Mvc)\JavaWeb开发实战教程(JSP+Servlet+Mvc)\《第五章 会话技术Cookie》课程资料\讲义\补5-前端安全系列之二：如何防止CSRF攻击？.md)
...
用户一旦被诱骗发送这个表单，银行网站就会收到带有正确 Cookie 的请求。为了防止这种攻击，表单一般都带有一个随机 token（这就属于防止CSRF攻击的手段了，别的防御策略可以去看《补2-CSRF+同源策略以及其他防御CSRF的策略+CORS.md》），告诉服务器这是真实请求。
