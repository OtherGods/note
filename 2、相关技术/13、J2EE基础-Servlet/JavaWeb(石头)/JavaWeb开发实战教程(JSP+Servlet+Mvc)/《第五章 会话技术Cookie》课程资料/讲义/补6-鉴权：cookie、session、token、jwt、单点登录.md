来自掘金：[前端鉴权的兄弟们：cookie、session、token、jwt、单点登录](https://juejin.cn/post/6898630134530752520)

# 前端鉴权的兄弟们：cookie、session、token、jwt、单点登录

[![img](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\09c4d2e238af48cebbb5408a1e3c2bde~100x100.awebp)](https://juejin.cn/user/1046390798562589)

[几木_Henry![lv-4](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\lv-4.a78c420.png)](https://juejin.cn/user/1046390798562589)

2020年11月24日 18:08 · 阅读 31522
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\典型的 session 登陆-验证流程.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\这是它种的 cookie.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\token 的流程是这样的.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\存成这样.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\例子.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\这串东西是怎么生成的呢？看图.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\有了 refresh token 后，几种情况的请求流程变成这样.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\百度也就是这么干的.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\一次「从 A 系统引发登录，到 B 系统不用登录」的完整流程.png)
...
![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\看这里.png)

对浏览器来说，SSO 域下返回的数据要怎么存，才能在访问 A 的时候带上？浏览器对跨域有严格限制，cookie、localStorage 等方式都是有域限制的。

这就需要也只能由 A 提供 A 域下存储凭证的能力。一般我们是这么做的：

![](D:\Tyora\AssociatedPicturesInTheArticles\补6-鉴权：cookie、session、token、jwt、单点登录\一般我们是这么做的.png)
