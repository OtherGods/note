首先，我们就是可以想办法减少 Redis 挂了的概率，以及降低挂了可能带来的影响，这主要就是哨兵模式、集群模式的一些东西了，这不展开了。（本文内容来自于我的[八股文](http://nfturbo.wiki/bagu.html)，我针对其中部分内容做了修改，加了些本项目内容进去。）

在秒杀环节中，我们借助 Redis 来抗库存扣减流量，对于单个请求，如果Redis 扣减之后，Redis 挂了，其实影响不大，因为后续请求会正常去扣减数据库。

但是，一旦Redis 挂了，后续的消息也丢了，这影响就大了。

这种情况的话，还是比较难解决的，就是屋漏偏逢连阴雨了。不仅 Redis 挂了，MQ 也挂了，或者应用也跟着挂了。那业务基本上就要停摆了。等恢复。而且恢复之后，还不能立即就对外提供服务，还需要人工介入来核对数据，确保数据无误之后再对外提供服务，这也是为啥有些大公司，服务挂了之后要很久才恢复，大部分时间是在恢复数据的。。。

那么，如果只是 Redis 挂了，或者 Redis+MQ 挂了，应用还没挂的话，有啥办法呢？

首先我们需要**有机制可以发现 Redis 挂了**，最常见的就是加监控了，一般对于 Redis 自己可以做监控，我们在业务中也可以基于 Redis 调用的成功率做一些监控，当发现长时间不可用，或者超时之后，就可以认为是 Redis 挂了，就需要考虑应急机制了。
