在很多项目中都需要有严格的权限控制，需要一套鉴权系统，比较典型的就是 `RBAC`，但是我们是一个电商项目，在权限这块没那么复杂。

我们实现了一套简单的权限控制体系，但是用到的技术方案也是业内通用的方案，即使用 `SaToken` 进行鉴权，并且在`SpringCloudGateway`中实现统一鉴权。
...
在 gateway 的应用中，我们定义一个`StpInterfaceImpl`，让他实现`cn.dev33.satoken.stp.StpInterface`这个接口。在其中实现权限和角色的获取：
```java
/**  
 * 自定义权限验证接口  
 *  
...
这里的代码很简单，首先是角色的判断，如果是 `ADMIN` 的角色，那么我们就在`getRoleList`中返回 `ADMIN` 给他，如果不是，则返回 `CUSTOMER` 的角色给他。
...
并且为了让错误提示更加友好，我们定义了一个`getSaResult`方法，来对错误信息进行处理，这里为啥不用`GlobalWebExceptionHandler`这种形式呢？

**主要是因为这种==*全局异常捕获*只有在 `Spring MVC` 应用中才能生效==，而 `gateway` 不是 `web` 应用而是 `webflux` 应用**。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202502162200595.png)
