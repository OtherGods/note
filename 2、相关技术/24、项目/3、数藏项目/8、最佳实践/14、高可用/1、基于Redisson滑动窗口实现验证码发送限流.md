...
public NoticeResponse generateAndSendSmsCaptcha(String telephone) {  
    Boolean access = slidingWindowRateLimiter.tryAcquire(telephone, 1, 1 * 60);  
...
这里，调用我们的 SlidingWindowRateLimiter 进行了一个限流，限流的 key 是手机号，时间是限制的1分钟内只能发送1条。
[8、限流组件：nft-turbo-limiter](2、相关技术/24、项目/3、数藏项目/9、公共组件/8、限流组件：nft-turbo-limiter.md)
...
其实，主要是因为我们用的限流器有一个特殊的前提条件，那就是在一定的时间范围内，我们允许的请求量为1，当请求量为1的时候，RRateLimiter的实现其实就是一个滑动窗口。（具体原理大家可以看一下视频，视频中有一期专门讲的这个，请原谅我，文字表述实在是太复杂了）

视频标题：滑动窗口还是令牌桶？限流器实现原理解析
