...
所以当我们需要在存在线程池的场景中使用ThreadLocal 进行参数的传递的时候，就需要借助阿里开源的TTL：[https://github.com/alibaba/transmittable-thread-local](https://github.com/alibaba/transmittable-thread-local)

在我们的项目中也用到了TTL，场景是这样的：

我们给大家提供了一个 MockPayChannelServiceImpl，用于在不对接第三方支付的时候，直接把支付渠道给他 mock 掉。
