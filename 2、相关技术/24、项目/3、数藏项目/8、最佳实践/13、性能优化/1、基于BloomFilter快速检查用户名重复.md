在我们的项目中，用户名是不能重复的，因为在数藏项目中，用户名的唯一性很重要，因为藏品要做溯源，要确保整个链路上的参与者的唯一性，虽然用户 id 也是唯一的，但是页面上展示的时候不能用 id 呀。
...
它的基本原理是利用多个哈希函数，将一个元素映射成多个位，然后将这些位设置为 1。当查询一个元素时，如果这些位都被设置为 1，则认为元素 <font color = "red" size = 5>可能</font> 存在于集合中，否则 <font color = "red" size = 5>肯定</font> 不存在。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202503032129402.png)
