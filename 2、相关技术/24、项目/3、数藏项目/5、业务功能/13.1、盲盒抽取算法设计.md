...
这里面，最基本的即使纯随机方式，其他的方式都是在纯随机的基础上衍生出来的。

在我们的项目中，我们目前只实现了一种随机方式，但是预留了支持多种算法的口子。
...
当然，新算法想要生效，还需要遵守一个约定，那就是你的 Service 的名字的前缀，需要和BlindAllotBoxRule的枚举项能对应的上，这样才会自动的路由，否则需要手动修改cn.hollis.nft.turbo.box.domain.service.BlindBoxRuleServiceFactory#get
```java
...
通过 `SELECT xxx FROM xxx WHERE xxx ORDER BY RAND() LIMIT 1` 实现随机获取一个，并且通过`WHERE blind_box_id = #{blindBoxId} AND state = #{state}`来过滤获取到指定的盲盒中还未被别人开盒的商品。
