...
首先，未登录用户能访问哪些页面，是我们通过`checkLogin`实现的，这部分在`SaTokenConfigure`中配置的：
```java
...
其实重要的就是`StpInterfaceImpl`，是`StpInterface`的实现类，`StpInterface`是 `SA-Token` 中的一个接口，会在用户访问时执行里面的方法获取到用户的角色和权限列表：
```java
/**  
 * 自定义权限验证接口  
 *  
 * @author Hollis */@Component  
...
1、`StpUtil.getSessionByLoginId`为啥能拿到用户信息？
```java
UserInfo userInfo = (UserInfo) StpUtil.getSessionByLoginId(loginId).get((String) loginId);
```
2、返回的权限和角色列表在哪用？

# `StpUtil.getSessionByLoginId`为啥能拿到用户信息？

因为我们在用户登录的时候，往 session 里面 put 的用户信息， 代码如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202411142304030.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202411142309228.png)
