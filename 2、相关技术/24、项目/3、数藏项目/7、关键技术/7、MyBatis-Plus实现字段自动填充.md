...
基于MyBatis-Plus的MetaObjectHandler我们可以很简单的实现这个逻辑，比如：
```java
package cn.hollis.nft.turbo.datasource.handler;  
...
/**  
 * @author Hollis */public class DataObjectHandler implements MetaObjectHandler {  
    @Override  
...
通过以上代码，我们实现了在 insert 的时候，自动初始化gmtCreate、gmtModified、deleted以及lockVersion。在 update 的时候，自动修改gmtModified。

有人说，为啥不直接用数据库字段的默认值，有些可以，比如 lockVersion 和deleted ，但是有些就不太行，比如每次更新的时候我都希望他自动帮我更新创建时间。就没办法用默认值实现。

然后，再把这个DataObjectHandler配置成一个 bean 即可：
```java
