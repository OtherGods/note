...
我们项目中就是用 H2作为内存数据库进行的单元测试。接入文档：
[1、H2内存数据库接入](2、相关技术/24、项目/3、数藏项目/4、框架接入/1、H2内存数据库接入.md)

如我们的OrderManageServiceTest，虽然是 Servie 的单测，但是不会写到我们真正的MySQL数据库中，而是从 H2中进行 CRUD。
```java
/**  
 * @author Hollis */public class OrderManageServiceTest extends OrderBaseTest {  
    @Autowired  
