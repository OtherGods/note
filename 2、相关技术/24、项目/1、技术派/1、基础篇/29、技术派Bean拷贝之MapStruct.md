大家好，我是二哥呀。今天由我来给大家讲一下 MapStruct 在技术派中的应用。简单来说，MapStruct 就是一个 Java Bean 的映射器，我们只需要在一个 XXXStructMapper 的接口中定义好方法，MapStruct 就会在编译的时候生成相应的实现类，这个实现类中包含了具体的映射代码，极大地提高了我们编码的效率，省去了大量的模板代码。
...
这样的样板代码写多了，你会感觉毫无价值，自己就像一个复制粘贴的 CRUD Boy。
...
3. **toDTO方法**
   这个方法定义了一个转换。作用大家都懂，我就不废话了。

前后对比一下，是不是体感完全不一样。

# 2、MapStruct的基本用法

**第一步，在 pom.xml 文件中增加 MapStruct 的 Maven 坐标。**
```xml
<!-- 引入 mapstruct -->
<dependency>
...
**第二步，定义映射器接口，就像前面大家看到的 ArticleStructMapper 接口，我们稍微来一个复杂的。**
```java
@Mapper
...
3. **toDo 方法：**
   对于 freeStartTime 和 freeEndTime，因为它们在 ColumnReq 中是 Long 类型的时间戳，而在 ColumnInfoDO 中是 Date 类型，所以使用了 expression 属性进行转换。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409162358983.png)

安装完成后，可以直接在 @Mapper  接口和它的实现类之间快速导航。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409162358461.png)

比如说点击上图中的跳转小图标，就可以直接到实现类。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409162359612.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409170001463.png)

通过终端可以看到 class 文件的后缀。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409170003591.png)

OK，我们直接来看 class 文件的内容，当然是反编译后的，Intellij IDEA 可以直接打开，先是 SimpleSourceDestinationMapper。

![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409170003457.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409170006681.png)
