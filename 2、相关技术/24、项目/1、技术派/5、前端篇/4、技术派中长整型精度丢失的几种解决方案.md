...
接下来我们以技术派中选择的Thymeleaf模板渲染引擎，来介绍一下对于大整数的精度丢失问题的几种解决方案
...
直接访问，表现如下
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202410031744199.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202410032131915.png)
...
Thymeleaf模板的参数传递，并不是通过HttpMessageConverter来实现的，数据转换的实现主要是靠IStandardJavaScriptSerializer

# 4、Thymeleaf长整型精度丢失问题解决方案

既然直接返回json数据可以通过修改序列化的转换方式来实现，那么Thymeleaf按照这个思路，应该也是可行的

直接通过debug，我们可以知道Thymeleaf默认使用的是JacksonStandardJavaScriptSerializer来对js传递的对象进行序列化
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202410032132892.png)

从JacksonStandardJavaScriptSerializer的实现来看，比较遗憾的是它并没有支持长整型转字符串，也没有预留给我们进行注册Module的口子
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202410032137272.png)
...
2. 前端接收时，以字符串方式接收长整形
3. 后端针对json返回，通过注册自定义的HttpMessageConverter做统一的长整型格式化转换
4. 对于Thymeleaf模板渲染引擎，通过修改IStandardJavaScriptSerializer支持长整型的格式转换
