...
SpringBoot程序启动加载流程里，会执行SpringApplication.run中的`prepareEnvironment()`方法进行配置的初始化，那么初始化过程每一步都做了什么呢？
...
看看它的配置加载流程步骤：
1. 创建 **环境对象** `ConfigurableEnvironment` 用于存储环境参数；
2. `configureEnvironment` 方法加载默认的 `application.properties` 和 `application.yml` 配置文件；以及用户指定的配置文件，将其封装为 **PropertySource** 添加到环境对象中；
3. `attach()`： 加载所有的系统属性，并将它们添加到环境对象中；
4. `listeners.environmentPrepared()`： 发送环境参数配置已经准备就绪的监听通知；
5. `moveToEnd()`： 将 **系统默认** 的属性源中的所有属性值移到环境对象的队列末尾，这样用户自定义的属性值就可以覆盖默认的属性值。
6. `bindToSpringApplication`： 应用程序的属性绑定到 Bean 对象上；
7. `attach()`： 再次加载系统配置，以防止被其他配置覆盖；
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401100008569.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401100016907.png)
...
毫无悬念，成功拿到配置数据。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401112311968.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401112312079.png)
...
@Value 注解加到`final`关键字上同样也无法获取属性值，因为 final 变量必须在声明时或在代码块中进行初始化，并且一旦被赋值便不能再次更改。而 @Value 注解是在 bean 实例化之后才进行属性注入的，因此无法在构造方法中初始化 final 变量。
```java
@Slf4j
@SpringBootTest
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401141636525.png)
...
成功获取配置了
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401141740174.png)

但是当我试图加载`.yaml`文件时，启动项目居然报错了，经过一番摸索我发现，@PropertySources 注解只内置了`PropertySourceFactory`适配器。也就是说它只能加载`.properties`文件。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202401141741325.png)
...
转载自：[6 种方式读取 Springboot 的配置，老鸟都这么玩（原理+实战）](https://www.cnblogs.com/chengxy-nds/p/17484997.html)