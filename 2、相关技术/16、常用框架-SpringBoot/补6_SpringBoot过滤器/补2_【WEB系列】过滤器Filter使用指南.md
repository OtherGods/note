...
本文将介绍些什么是Filter，以及在SpringBoot中使用Filter的一般姿势与常见问题。
...
Filter，过滤器，属于Servlet规范，并不是Spring独有的。其作用从命名上也可以看出一二，拦截一个请求，做一些业务逻辑，然后可以决定请求是否可以继续下分发，落到其他的Filter或者对应的Servlet

简单描述一下http请求过来后一个Filter的工作流程：
1. 首先进入Filter，执行相关业务逻辑
2. 若判定同行，则进入Servlet逻辑，Servlet执行完毕之后，又返回Filter，最后再返回给请求方
3. 判定失败，直接返回，不需要将请求发送给Servlet

> 上面这个过程中与AOP中的@Around环绕切面的作用差不多。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202307300920808.png)
  

### 2.1.3、FilterRegistrationBean注册过滤器
...
### 2.2.1、Filter依赖Bean注入问题
如果有小伙伴使用`Spring MVC + web.xml`方式来定义Filter，就会发现自定义的FIlter中无法通过@Autowired方式来注入Spring的bean
> 这个问题基于spring4、Servlet2+
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202307300941433.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202307300952909.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202307300955659.png)

1. OrderFiler是我们手动注册并设置优先级为 -1；
2. ReqFilter, AuthFilter通过 WebFillter方式注册，默认优先级为`2147483647`，相同优先级的情况下，根据名字先后顺序来决定。
...
转载自：[【WEB系列】过滤器Filter使用指南](https://spring.hhui.top/spring-blog/2019/10/16/191016-SpringBoot%E7%B3%BB%E5%88%97%E6%95%99%E7%A8%8Bweb%E7%AF%87%E4%B9%8B%E8%BF%87%E6%BB%A4%E5%99%A8Filter%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8D%97/)
