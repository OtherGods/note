...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202307231147117.png)
...
第一种方式：其实也是很早之前常用的一种方式，只需要在代码添加如下：
```java
...
那么完整的配置如下：
```properties
logging.level.root=DEBUG logging.level.com.example.demo=INFO
```
...
## 5.1、configuration节点
这是一个根节点，其中各个属性如下：
1. scan：当此属性为true时，配置文件如果发生改变，将会被重新加载，默认值为true
2. scanPeriod：设置检测配置文件是否有修改的时间间隔，如果没有给出时间单位，默认单位是毫秒。当scan为true时，此属性生效。默认的时间间隔为1分钟。
...
4. `rollingPolicy`: 日志回滚策略，在这里我们用了TimeBasedRollingPolicy，基于时间的回滚策略,有以下子节点：
	1. `fileNamePattern`：必要节点，可以用来设置指定时间的日志归档。
	2. `maxHistory`：可选节点，控制保留的归档文件的最大数量，超出数量就删除旧文件，例如设置为30的话，则30天之后，旧的日志就会被删除
	3. `totalSizeCap`：可选节点，用来指定日志文件的上限大小，例如设置为3GB的话，那么到了这个值，就会删除旧的日志
...
来自：[Spring Boot 第三弹，一文带你搞懂日志如何配置？](https://juejin.cn/post/6877473104403759118)