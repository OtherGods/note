...
首先ZipList是一个**压缩的数据结构**，它的**每个元素都是连续存储的（和数组有点像）**，因此内存的使用非常**紧凑**。与其他数据结构相比，ZipList在小规模数据存储时显著减少了内存占用。

而在内存方面，**SkipList 相比于 ZipList 内存开销要大得多**，因为它是通过多个**链表**层级来实现的，每个元素需要额外的指针来维护层级结构。
...
而**SkipList 采用多级索引结构，它的查找、插入、删除操作的时间复杂度是 O(log N)，远远优于 ZipList 的 O(N)**。
