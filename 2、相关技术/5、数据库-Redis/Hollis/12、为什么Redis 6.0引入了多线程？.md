#select、poll是同步阻塞的 #充分利用多CPU的优势 #redis的性能瓶颈主要在IO上  
...
![1.jpg](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404201024166.jpg)

但随着越来越复杂的业务场景，有些公司动不动就上亿的交易量，因此需要更大的 QPS。

为了提升QPS，很多公司的做法是部署Redis集群，并且尽可能提升Redis机器数。但是这种做法的资源消耗是巨大的。

而经过分析，限制Redis的性能的主要瓶颈出现在网络IO的处理上，虽然之前采用了多路复用技术。但是我们前面也提到过，多路复用的IO模型本质上仍然是 <font color = "red">同步阻塞型</font> IO模型。
[3、操作系统的IO模型有哪些？](4、操作系统/Hollis/3、操作系统的IO模型有哪些？.md)

下面是多路复用IO中select函数的处理过程：
![1.jpg](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404201838793.jpg)

从上图我们可以看到，**在多路复用的IO模型中，在处理网络请求时，调用 select （其他函数同理）的过程是阻塞的，也就是说这个过程会阻塞线程，如果并发量很高，此处可能会成为瓶颈（但是select函数有超时参数，ChatGPT认为这是IO多路复用是同步非阻塞的原因）。**
