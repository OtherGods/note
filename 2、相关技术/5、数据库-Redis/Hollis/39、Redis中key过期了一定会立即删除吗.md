...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507112344379.png)
...
主动删除的优点：
1. ~~**及时释放内存**：主动删除能够及时地释放过期键占用的内存，避免内存空间被长时间占用，从而降低了内存使用率。~~(这里应该是即使删除键吧，在[19、Redis 的过期策略是怎么样的？](2、相关技术/5、数据库-Redis/Hollis/19、Redis%20的过期策略是怎么样的？.md)里说主动删除不会立即释放内存)
