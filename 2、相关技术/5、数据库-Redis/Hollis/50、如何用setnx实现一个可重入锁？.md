...
[51、什么是可重入锁，怎么实现可重入锁？](2、相关技术/2、JUC/Hollis/Java并发/51、什么是可重入锁，怎么实现可重入锁？.md)

在Redis中，最简单的方式就是使用setnx来实现一个分布式锁了，但是如果我想要实现一个具有重入功能的锁，那么用setnx如何实现呢？

首先，我们需要有一个标识来识别出一个线程，这里可以是线程ID，分布式的traceId [23、如何实现应用中的链路追踪？](2、相关技术/21、分布式/Hollis/23、如何实现应用中的链路追踪？.md)，或者是一个唯一的业务ID都可以。
