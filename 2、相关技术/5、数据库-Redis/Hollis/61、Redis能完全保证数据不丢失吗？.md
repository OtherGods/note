...
[60、RDB和AOF的写回策略分别是什么？](2、相关技术/5、数据库-Redis/Hollis/60、RDB和AOF的写回策略分别是什么？.md)
...
> <font size=5 color="red">操作系统缓冲区</font>，通常指的是操作系统用于管理数据输入输出（I/O）的一种内存区域。当**程序进行文件写入操作时，数据通常首先被写入到这个缓冲区，而不是直接写入到硬盘**。
