#热key问题 #流量太大导致物理资源无法支撑 #热key拆分 #多级缓存 #限流 

# 典型回答

当我们使用Redis作为存储时，如果发生一些特殊情况，比如明星官宣的突发事件，世界杯等重大活动，双十一的活动秒杀等等，就会出现特别大的流量，并且会导致某些热词、商品等被频繁的查询和访问。

**如果在同一个时间点上，Redis中的==同一个key被大量访问==，就会导致==流量过于集中==，使得很多==物理资源无法支撑，如网络带宽、物理存储空间、数据库连接等==。**
...
**在事中解决方面，主要可以考虑：==热点key拆分==、==多级缓存==、热key备份、==限流==等方案来解决。**
...
将一个热key拆分成多个key，在每一个Key后面加一个后缀名，然后把这些key分散到多个实例中【这里的意思应该是将这些拆分后的key分散到不同的redis集群分片上】。
