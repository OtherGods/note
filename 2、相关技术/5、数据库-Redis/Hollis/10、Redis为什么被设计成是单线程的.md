#redis单线程 #redis的执行命令的操作基本都是基于内存 #避免了多线程切换带来的压力 #想要提升IO复用率可以通过多路复用 
...
**而==多线程的目的，就是通过并发的方式来提升I/O的利用率和CPU的利用率==**。
...
**虽然，采==用多线程可以帮助我们提升CPU和I/O的利用率==，但是多线程带来的并发问题也给这些语言和框架带来了更多的复杂性。而且，多线程模型中，==多个线程的互相切换也会带来一定的性能开销==。**
...
1. Redis 操作**基于内存**，绝**大多数操作的性能瓶颈不在 CPU**
2. 使用单线程模型，可维护性更高，开发，调试和维护的成本更低
3. 单线程模型，**避免了线程间切换带来的性能开销**
4. 在**单线程中使用多路复用 I/O**技术也能提升Redis的I/O利用率

还是要记住：Redis并不是完全单线程的，只是有关键的键值对读写是由一个线程完成的。

## 2.2、Redis的多路复用

[11、为什么Redis设计成单线程也能这么快？](2、相关技术/5、数据库-Redis/Hollis/11、为什么Redis设计成单线程也能这么快？.md)

## 2.3、为什么Redis6.0引入多线程

[12、为什么Redis 6.0引入了多线程？](2、相关技术/5、数据库-Redis/Hollis/12、为什么Redis%206.0引入了多线程？.md)
