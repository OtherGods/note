#缓存一致 #续写并发 #写写并发 #删除缓存 #先更新数据库后删缓存 #先删缓存后更新数据库 #延迟双删 

# 典型回答

[24、什么情况下会出现数据库和缓存不一致的问题？](2、相关技术/5、数据库-Redis/Hollis/24、什么情况下会出现数据库和缓存不一致的问题？.md)
...
<font color="red" color="red" size=5>注意：这里更新数据库时的sql：若是直接赋值则有问题<font color="blue">（覆盖掉其他SQL的修改）</font>，若是在sql语句中基于原有值修改后赋值则没问题<font color="blue">（基于其他SQL的修改进行修改）</font></font>
```sql
-------------------- !!!这里以库存扣减举例!!!--------------------
-- 库存表：collection
-- 可售库存：saleable_inventory
-- 原有库存：quantity

-- 这两段SQL的区别主要就是 **加到多少** 和 **加多少**
-- 这两段SQL的区别主要就是 **减到多少** 和 **减多少**

-- 如果更新的sql是直接给字段赋值，就会出现覆盖的问题
UPDATE collection 
	SET saleable_inventory = 10 
WHERE 
	id = #{id} 
	and <![CDATA[saleable_inventory >= #{quantity}]]>

-- 如果更新的sql是基于原值做修改，就类似于库存扣减或增加，则不会出现覆盖的问题
UPDATE collection 
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506301637102.png)

### 先更新缓存，后写数据库：

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506301639333.png)

<font color="red" size=5>但是，如果是做缓存的删除的话，在写写并发的情况下，缓存中的数据都是要被清除的，所以就不会出现数据不一致的问题；【在读写并发的情况下会出现问题，具体看：<font color="blue">先写数据库还是先删缓存</font> 小节】</font>
...
但是，先删除缓存后写数据库的这种方式，会无形中**放大"读写并发"导致的数据不一致的问题**。我们知道，当我们使用了缓存之后，一个读的线程在查询数据的过程是这样的：
...
**读写并发的时序如下：**
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506301645135.png)
...
**这种方式在 "读写并发" 的场景下也会导致不一致的情况，读写并发的时序如下：**
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506301700553.png)

那么怎么解决呢？

### 延迟双删

[42、为什么需要延迟双删，两次删除的原因是什么？](2、相关技术/5、数据库-Redis/Hollis/42、为什么需要延迟双删，两次删除的原因是什么？.md)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506301702949.png)
