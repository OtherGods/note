...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072204414.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072206303.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072210604.png)

常见的路由断言规则如下图所示：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072212980.png)

# Spring Cloud Gateway 的路由和断言是什么关系？

Route 路由和 Predicate 断言的对应关系如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072216542.png)

- **一对多**：<font color="red" size=5>一个路由规则可以包含多个断言</font>。如上图中路由 Route1 配置了三个断言 Predicate。
- **同时满足**：<font color="red" size=5>如果一个路由规则中有多个断言，则需要同时满足才能匹配</font>。如上图中路由 Route2 配置了两个断言，客户端发送的请求必须同时满足这两个断言，才能匹配路由 Route2。
- **第一个匹配成功**：<font color="red" size=5>如果一个请求可以匹配多个路由，则映射第一个匹配成功的路由</font>。如上图所示，客户端发送的请求满足 Route3 和 Route4 的断言，但是 Route3 的配置在配置文件中靠前，所以只会匹配 Route3。
...
其实这些复杂的步骤并不需要我们手动实现，通过 Nacos Server 和 Spring Cloud Alibaba Nacos Config 即可实现配置的动态变更，[官方文档](https://github.com/alibaba/spring-cloud-alibaba/wiki/Nacos-config)。

# Spring Cloud Gateway 的过滤器有哪些？

过滤器 Filter 按照请求和响应可以分为两种：
- **Pre 类型**：在请求**被转发到微服务之前**，对请求进行拦截和修改，例如参数校验、权限校验、流量监控、日志输出以及协议转换等操作。
...
常见的局部过滤器如下图所示：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072224758.png)
...
常见的全局过滤器如下图所示：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506072229261.png)
...
- 全局异常处理: [https://zhuanlan.zhihu.com/p/347028665](https://zhuanlan.zhihu.com/p/347028665)

转载自：[Spring Cloud Gateway常见问题总结](https://javaguide.cn/distributed-system/spring-cloud-gateway-questions.html#spring-cloud-gateway-%E5%A6%82%E4%BD%95%E8%87%AA%E5%AE%9A%E4%B9%89%E5%85%A8%E5%B1%80%E5%BC%82%E5%B8%B8%E5%A4%84%E7%90%86)