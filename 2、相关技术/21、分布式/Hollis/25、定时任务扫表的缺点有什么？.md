...
[11、如何基于本地消息表实现分布式事务？](2、相关技术/21、分布式/Hollis/11、如何基于本地消息表实现分布式事务？.md)
...
[50、区分度不高的字段建索引一定没用吗？](2、相关技术/4、数据库-MySQL/Hollis/50、区分度不高的字段建索引一定没用吗？.md)
...
[2、订单到期关闭如何实现](8、场景题/Hollis/2、订单到期关闭如何实现.md)
...
```java
private static ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("hollis-pool-%d").build();
...
[53、为啥不要在事务中做外部调用？](8、场景题/Hollis/53、为啥不要在事务中做外部调用？.md)

[4、基于Spring Event，实现同步转异步，解决定时任务扫表导致数据库连接池不够的问题](9、项目难点&亮点/4、基于Spring%20Event，实现同步转异步，解决定时任务扫表导致数据库连接池不够的问题.md)
