#幂等 #幂等的两种情况 #幂等方案 #数据库唯一约束 #数据库锁 #token  #分布式锁 
# 1、什么是幂等性？
...
<font color="red" size=5>幂等存在两种情况</font>
1. **==多次请求执行时重复==**：第一个请求还未执行完第二个请求就开始执行
   - *==**解决方式**：本文的方案都可以解决这种情况==*
2. **==多次请求执行时不重复==**：第一个请求执行成功，但执行超时了，调用端在第一个请求成功执行后，过了一段时间才发送第二个请求，第二个请求执行时第一个请求已经成功执行完毕
   - *==**解决方式**：数据库唯一约束、数据库锁等可以持久化到磁盘的方式==*，比如表唯一索引存token（代码：[3、基于Redis+MQ+数据库实现高并发扣减](2、相关技术/24、项目/3、数藏项目/8、最佳实践/2、秒杀（热点扣减）/3、基于Redis+MQ+数据库实现高并发扣减.md) ）记录状态转换；分布式锁、防重Token解决不了这种情况，因为第一次请求执行完毕就会释放分布式锁，第二次请求可以成功获取分分布式锁
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132123115.png)

# 6、幂等方案

## 6.1、方案一：数据库唯一主键实现幂等性
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132222745.png)

主要流程如下：
1. 客户端执行创建请求，调用服务端接口。
2. 服务端执行业务逻辑，生成一个分布式 `ID`，将该 ID 充当待插入数据的主键，然 后执数据插入操作，运行对应的 `SQL` 语句。
	1. 对同一请求参数生成的分布式id是唯一的且相同，所以第二次请求生成的订单id再插入就会触发唯一索引了
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132227132.png)

例如，存在如下的数据表中：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132228479.png)

为了每次执行更新时防止重复更新，确定更新的一定是要更新的内容，我们通常都会添加一个 `version` 字段记录当前的记录版本，这样在更新时候将该值带上，那么只要执行更新操作就能确定一定更新的是某个对应版本下的信息。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132229152.png)
...
## 6.3、方案三：防重Token令牌实现幂等性

这种方式类似于使用**Redis实现分布式锁**解决幂等的方案
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132234377.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404132322316.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202404141009146.png)


作者：码猿技术专栏  
链接：[https://juejin.cn/post/6906290538761158670](https://juejin.cn/post/6906290538761158670)
来源：稀土掘金  
