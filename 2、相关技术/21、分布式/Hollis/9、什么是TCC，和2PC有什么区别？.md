...
TCC是Try-Confirm-Cancel的缩写，它是一种分布式事务解决方案，采用了基于业务逻辑的补偿机制，将整个分布式事务分解为若干个子事务，每个子事务都有一个`try`、`confirm`和`cancel`三个操作，通过这些操作来实现分布式事务的执行和回滚
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202411241546283.png)
...
2. 存在**悬挂事务**问题：TCC的实现方式存在悬挂事务的问题，即在执行过程中可能会有部分子事务成功(子事务先接收到Cancel再接收到Try)，而其他子事务失败，导致整个事务无法回滚或提交。
...
[22、TCC的空回滚和悬挂是什么？如何解决？](2、相关技术/21、分布式/Hollis/22、TCC的空回滚和悬挂是什么？如何解决？.md)

# 扩展知识
## Confirm/Cancel失败了怎么办？

[32、TCC中，Confirm或者Cancel失败了怎么办？](2、相关技术/21、分布式/Hollis/32、TCC中，Confirm或者Cancel失败了怎么办？.md)
