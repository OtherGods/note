对照：[33、自定义雪花算法生成业务ID](2、相关技术/24、项目/1、技术派/2、进阶篇/33、自定义雪花算法生成业务ID.md)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202412151612048.png)
...
一旦发生这种情况，简单粗暴的做法是抛异常，**发现时钟回调了，就直接抛异常出来**。例如：`hutool`框架钟获取雪花ID的方式就是这样，如果回拨时间超过默认的`2s`就抛异常，否则使用上次保存的时间戳配合自增的序号（小于4096，1ms生成4096个序号）来生成新的雪花ID，如果自增序号超过4096就等待下一毫秒再生成雪花ID；`hutool 5.8.22`源码如下：
```java
/**  
 * 下一个ID  
 * 
...
public synchronized long nextId() {  
    ...
    if (timestamp == this.lastTimestamp) {  
       ...
       // 1ms内序号生成超过4096，则调用方
       // 法tilNextMillis等待到上一次生成雪花ID的下一毫秒
       if (sequence == 0) {  
...
另外还有一种做法就是**发现时钟变小了，就拒绝ID生成请求**，等到时钟恢复到上一次的ID生成时间点后，再开始生成新的ID。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202412151618053.png)
