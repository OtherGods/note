...
[53、为什么不能在try-catch中捕获子线程的异常？](2、相关技术/2、JUC/Hollis/Java并发/53、为什么不能在try-catch中捕获子线程的异常？.md)
...
我们还可以使用UncaughtExceptionHandler，UncaughtExceptionHandler 是 Java 中的一个接口，**用于处理未捕获异常**，即那些没有被 try-catch 块捕获的异常。它允许定义自定义异常处理器，以便在线程出现未捕获异常时采取特定的操作。
