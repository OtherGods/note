...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 锁种类             | **==可重入锁==**、[非公平锁](2、相关技术/2、JUC/Hollis/Java并发/68、sychronized是非公平锁吗，那么是如何体现的？.md)、**==互斥锁==**                                                        | **==可重入锁==**、**==互斥锁==**、公平锁、非公平锁、可中断锁                                                                                                                                                                                                              |
...
| 条件队列            | `ObjectMonitor`的`waitSet`；<br><br>通过 `Object.wait()`, `notify()`, `notifyAll()` 操作，一个锁只能有一个等待队列。参考代码：[Synchronized](34、三个线程分别顺序打印0-100#Synchronized) | 通过 `Condition.await()`, `signal()`, `signalAll()` 操作，一个锁可以创建多个 `Condition`。<br><br>可以实现更精细的线程通知和唤醒。例如，生产者-消费者模型中，可以分别唤醒等待“非空”和“非满”条件的线程，避免了无效的“全通知”（`notifyAll()`），从而减少了不必要的线程争用和上下文切换，提升了性能。参考代码：[ReentrantLock](34、三个线程分别顺序打印0-100#ReentrantLock) |
