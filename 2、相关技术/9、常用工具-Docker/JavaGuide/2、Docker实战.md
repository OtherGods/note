...
开始之前，还是简单介绍一下 Docker，更多 Docker 概念介绍可以看前一篇文章[1、Docker核心概念总结](2、相关技术/9、常用工具-Docker/JavaGuide/1、Docker核心概念总结.md)。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403082219291.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091001740.png)
...
然后点击`Get Started`：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091003591.png)

在此处点击`Download for Windows`即可进行下载。

如果你的电脑是`Windows 10 64位专业版`的操作系统，则在安装 Docker 之前需要开启一下`Hyper-V`，开启方式如下。打开控制面板，选择程序：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091003359.png)

点击`启用或关闭Windows功能`：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091004635.png)

勾选上`Hyper-V`，点击确定即可：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091004967.png)

完成更改后需要重启一下计算机。

开启了`Hyper-V`后，我们就可以对 Docker 进行安装了，打开安装程序后，等待片刻点击`Ok`即可
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091004384.png)
安装完成后，我们仍然需要重启计算机，重启后，若提示如下内容：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091005834.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091016537.png)

在左上角的搜索框中输入`MySQL`并回车：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091017822.png)

可以看到相关 MySQL 的镜像非常多，若右上角有`OFFICIAL IMAGE`标识，则说明是官方镜像，所以我们点击第一个 MySQL 镜像：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091019966.png)

右边提供了下载 MySQL 镜像的指令为`docker pull MySQL`，但该指令始终会下载 MySQL 镜像的最新版本。

若是想下载指定版本的镜像，则点击下面的`View Available Tags`：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091021932.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091023455.png)

然后点击左上角的菜单，在弹窗的窗口中，将鼠标悬停在产品与服务上，并在右侧搜索容器镜像服务，最后点击容器镜像服务：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091024213.png)
...
<font color = "red">若想查看 Docker 中当前拥有哪些镜像</font>，则可以使用 `docker images` 命令。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091053633.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091056274.png)
...
若是要 <font color = "red">下载镜像</font>，则使用：
...
一般在下载镜像前我们需要 <font color = "red">搜索一下镜像有哪些版本</font> 才能对指定版本进行下载，使用指令：

```
docker search MySQL
```

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091103070.png)
...
若是查询的版本不存在，则结果为空：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091104765.png)

<font color = "red">删除镜像</font> 使用指令：
...
若需要 <font color = "red">通过镜像运行一个容器</font>，则使用：
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091117675.png)
...
此时外部就可以访问 Tomcat 了：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091120161.png)
...
该参数会 <font color = "red">将运行和非运行的容器全部列举出来</font>。
...
接下来是 <font color = "red">容器的停止、重启指令</font>，因为非常简单，就不过多介绍了。
...
当容器被停止之后，容器虽然不再运行了，但仍然是存在的，若是想 <font color = "red">删除容器</font> ，则使用指令：
...
当容器以后台的方式运行时，我们无法知晓容器的运行状态，若此时需要 <font color = "red">查看容器的运行日志</font>，则使用指令：
...
<font color = "red">查看容器内运行了哪些进程</font>，可以使用指令：

```
docker top 289cc00dc5ed
```

若是想 <font color = "red">与容器进行交互</font>，则使用指令：
...
现在我们已经能够进入容器终端执行相关操作了，那么该如何 <font color = "red">向 tomcat 容器中部署一个项目</font> 呢？
...
虽然使用 Docker 启动软件环境非常简单，但同时也面临着一个问题，我们无法知晓容器内部具体的细节，比如 <font color = "red">监听</font> 的端口、绑定的 ip 地址等等，好在这些 Docker 都帮我们想到了，只需使用指令：

```
docker inspect 923c969b0d91
```
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091154763.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091158802.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091200001.png)

这种方式设置的数据卷称为 <font color = "red">自定义数据卷</font>，因为数据卷的目录是由我们自己设置的，Docker 还为我们提供了 <font color="red">另外一种设置数据卷的方式</font>：
...
该指令能够 <font color = "red">将容器打包成一个镜像</font>，此时查询镜像：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091206857.png)

若是想<font color = "red">将镜像备份出来</font>，则可以使用指令：

```
docker save my_tomcat:1.0 -o my-tomcat-1.0.tar
```
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091208852.png)
...
```
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202403091209954.png)


著作权归JavaGuide(javaguide.cn)所有 基于MIT协议 原文链接：[https://javaguide.cn/tools/docker/docker-in-action.html](https://javaguide.cn/tools/docker/docker-in-action.html)
