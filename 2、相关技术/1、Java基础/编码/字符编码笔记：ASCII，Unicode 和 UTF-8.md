...
Unicode 当然是一个很大的集合，现在的规模可以容纳100多万个符号。每个符号的编码都不一样，比如，`U+0639`表示阿拉伯字母`Ain`，`U+0041`表示英语的大写字母`A`，`U+4E25`表示汉字`严`。具体的符号对应表，可以查询[unicode.org](https://www.unicode.org/ "unicode.org")，或者专门的[汉字对应表](http://www.chi2ko.com/tool/CJK.htm "汉字对应表")。

### **四、Unicode 的问题**

需要注意的是，Unicode 只是一个符号集，它<font color="red" size=5>只规定了<font color="blue" size=5>符号的二进制代码</font>，却没有规定这个二进制代码应该如何存储</font>。
...
UTF-8 的编码规则很简单，只有二条：**【正是这两条规则决定了计算即如何识别三个字节代表的是一个符号还是三个符号】**
...
跟据上表，解读 UTF-8 编码非常简单。如果一个字节的第一位是`0`，则这个字节单独就是一个字符；如果第一位是`1`，则连续有多少个`1`，就表示当前字符占用多少个字节。【这是计算机能够区分几个字节代表一个字符的基础】
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507190925969.png)
...
然后，用文本编辑软件[UltraEdit 中](https://www.google.cn/search?aq=t&oq=UltraEdit&complete=1&hl=zh-CN&newwindow=1&rlz=1B3GGGL_zh-CNCN216CN216&q=ultraedit+%E4%B8%8B%E8%BD%BD&btnG=Google+%E6%90%9C%E7%B4%A2&meta= "UltraEdit 中")的"十六进制功能"，观察该文件的内部编码方式。
...
转载自：[字符编码笔记：ASCII，Unicode 和 UTF-8 - 阮一峰的网络日志](https://www.ruanyifeng.com/blog/2007/10/ascii_unicode_and_utf-8.html "字符编码笔记：ASCII，Unicode 和 UTF-8 - 阮一峰的网络日志")