...
**==后续动态改变JVM参数（比如使用jinfo修改JVM的值），jps看不到修改后的值==**，`jinfo -flags`和`jcmd pid VM.flags`能看到修改后的值

在这里，在启动main方法的时候，我给jvm传递一个参数：-Dfile.encoding=UTF-8,执行`jps -v`：
```
...
需要更改jre安全策略
详细看51CTO视频：jps与jstatd与jvisualvm的联合实操
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508052127749.png)
