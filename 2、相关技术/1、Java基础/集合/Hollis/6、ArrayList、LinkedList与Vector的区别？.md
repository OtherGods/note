...
Vector和ArrayList在更多元素添加进来时会请求更大的空间。**Vector每次请求其大小的双倍空间**，而**ArrayList每次对size增长50%**。
...
3. 判断要申请的数组的容量（可能调用addAll方法） 与 老容量的1.5倍那个大【ensureCapacityInternal(int minCapacity)方法】，取最大的容量执行第四步
4. 之后，申请一个新容量数组（最小为老容量的1.5倍），并将老数组的元素复制到新数组中，扩容完成

## 如何利用List实现LRU？

LRU，即最近最少使用策略，基于时空局部性原理（最近访问的，未来也会被访问），往往作为**缓存淘汰的策略**，如Redis和GuavaMap都使用了这种淘汰策略。
...
[1、数组和链表有何区别？](12、数据结构/Hollis/1、数组和链表有何区别？.md)
