...
那么，为什么最终选定了0.75呢？因为`threshold=loadFactor*capacity`，并且capacity永远都是2的幂，为了保证负载因子（loadFactor） * 容量（capacity）的结果是一个整数，这个值是0.75(3/4)比较合理，因为这个数和任何2的幂乘积结果都是整数。


# 评论补充

hashmap的扩容因子为0.75的原因

==二项分布的概率质量函数通式==
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506201633139.png)

p(0)=(1-1/s)^n

1、当槽s数量为无穷大，且**一个槽bucket为空和非空的概率为0.5时，扩容因子为log2≈0.7，可以使得一个bucket为空和非空的概率为0.5**。
2、hashmap容量2^n，他的0.75倍为整数，是离log2最近的整数