...
在使用Comparable接口接口排序的时候，被排序的对象必须全部都实现Comparable接口。例如：`Arrays.sort( [ 元素都实现了Comparable接口 ] )`

在使用Comparator接口的时候，被排序的对象不需要全部都实现接口Comparator，只需要排序的时候将比较器的实现类的实例对象传入即可。例如：`Arrays.sort( [ 普通元素 ] , 实现了接口Comparator接口的实例对象  )`
