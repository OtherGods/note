...
不过可能有人会问，那不是需要一个next节点就行了，为什么还要prev【TreeNode继承Node后多加了一个prev字段记录前一个节点】点呢？这是因为当删除红黑树中的某个节点的时候，这个节点可能就是原始链表的中间节点，如果把该节点删除，只有next属性是没办法将原始的链表重新链接的（除非从头到尾遍历链表才能找到当前节点的前置节点，为了尽快定位，所以需要prev），所以就需要prev节点，找到上一个节点，重新成链
