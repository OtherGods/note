...
<font color="red" size=5>为了方便将取模运算转换成与运算（与运算这种位运算直接对内存数据操作，不需要转换成十进制，效率更高）</font>
...
[18、HashMap的hash方法是如何实现的？](2、相关技术/1、Java基础/集合/Hollis/18、HashMap的hash方法是如何实现的？.md)
...
从2进制角度来看，`X / 8`相当于 `X >> 3`，即把X右移3位，此时得到了`X / 8`的商，而被移掉的部分(后三位)，则是`X % 8`，也就是余数。
...
![1.jpg](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506201602312.jpg)
...
Step 1 怎么理解呢？**其实是对一个二进制数依次无符号右移，然后与原值取或**。其<font color="red" size=5>目的对于一个数字的二进制，从第一个不为0的位开始，把后面的所有位都设置成1</font>。

因为cap是int类型的，所以最多需要右移16位即可获取其最大值【Java中int类型是32位，在HashMap的tableSizeFor方法中一共向右移动了1 + 2 + 4 + 8 + 16 = 31位】。
...
**但是还有一种特殊情况套用以上公式不行，这些数字就是2的幂自身。如果cap=4套用公式的话。得到的会是 8，不过其实这个问题也被解决了，重点就在int n = cap - 1;这行代码中，HashMap会事先将用户给定的容量-1，这样就不会出现上述问题了**

总之，HashMap根据用户传入的初始化容量，<font color="red" size=5>利用无符号右移和按位或运算等方式计算出第一个大于该数的2的幂。</font>
...
loadFactor是装载因子，表示HashMap满的程度，默认值为`0.75f`，设置成0.75有一个好处，那就是0.75正好是`3/4`，而capacity又是2的幂。所以，两个数的乘积都是整数。
...
因为oldThr已经是`2^n`了，所以oldThr无符号左移之后，是`oldThr * 2`，自然也是`2^n`。至于后续HashMap是如何扩容的，请听下回分解～
...
[14、为什么HashMap的默认负载因子设置成0.75](2、相关技术/1、Java基础/集合/Hollis/14、为什么HashMap的默认负载因子设置成0.75.md)

## HashMap的初始值设为多少合适？

[15、HashMap的容量设置多少合适？](2、相关技术/1、Java基础/集合/Hollis/15、HashMap的容量设置多少合适？.md)
