...
Student student1 = new Student("hhy", 1);  
Student student2 = new Student("hhy", 2);  
Student student3 = new Student("hhy", 3);  
Student student4 = new Student("hhy", 4);  
Student student5 = new Student("hhy", 5);  
Student student6 = new Student("hhy", 6);  
...
5. 使用并发安全的集合类（**可以用**，但是需要转成线程安全的集合）
```java
...
6. 通过Stream的过滤方法，因为Stream每次处理后都会生成一个新的Stream，不存在并发问题，所以Stream的filter也可以修改list集合。**建议，简单高效**
```java
...
7. 通过removeIf方法，实现元素的过滤删除。从Java 8开始，List接口提供了removeIf方法用于删除所有满足特定条件的数组元素 **推荐**
```java
