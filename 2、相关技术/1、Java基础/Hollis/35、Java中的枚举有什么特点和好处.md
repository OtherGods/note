枚举类型是指由一组固定的常量组成合法的类型。Java中由关键字`enum`来定义一个枚举类型
```java
public enum Season {
...
如果传入的字符串**不匹配任何枚举常量，`valueOf` 会抛出 `IllegalArgumentException` 异常**，从而自动进行非法参数的校验。
```java
...
Enum类，Enum类中有两个属性：`name`、`ordinal`，分别是枚举对象名、枚举常量顺序
