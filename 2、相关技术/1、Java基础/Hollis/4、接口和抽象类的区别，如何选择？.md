...
**接口**和**抽象类**的区别其实挺多的。比如以下这些：
1. **方法定义**：接口和抽象类，最明显的区别就是接口只是定义了一些方法而已，在不考虑Java8中default方法情况下，接口中只有抽象方法，是没有实现的代码的。（Java8中可以有默认方法）
2. **修饰符**：
	- 抽象类中的修饰符可以有public、protected和private和`<default>`这些修饰符
	- 而接口中默认修饰符是public（接口中方法默认修饰符是`public abstract`、字段的默认修饰符是`public static final`，并且必须初始化），不可以使用其它修饰符。
```java
...
4. **继承和实现**：接口可以被实现，抽象类可以被继承。
5. **单继承，多实现**：一个类可以实现多个接口，但只能继承一个抽象类。接口支持多重继承，即一个接口可以继承多个其他接口。
```java
public interface HollisTestService extends InitializingBean, DisposableBean {}
```

6. **职责不同**：接口和抽象类的职责不一样。**接口主要用于==制定规范==**，因为我们提倡也经常使用的都是面向接口编程。而**抽象类主要目的是为了==复用==，比较典型的就是==模板方法模式、钩子方法==**。
...
[什么是模板方法模式，有哪些应用？](2、相关技术/10、设计模式/Hollis/什么是模板方法模式，有哪些应用？.md)
