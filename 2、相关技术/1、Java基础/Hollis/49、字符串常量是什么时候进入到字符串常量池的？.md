...
[33、字符串常量池是如何实现的？](2、相关技术/3、JVM/Hollis/33、字符串常量池是如何实现的？.md)

字符串常量池中的常量有两种来源：
1. 一种是**字面量会在编译期先进入到Class常量池**，然后再**在运行期进去到字符常量串池**
2. 一种就是**在运行期通过intern将字符串对象手动添加到字符串常量池**中。
...
对于 HotSpot 虚拟机来说，字符串字面量，和其他基本类型的常量不同，并**不会在类加载中的解析阶段填充并驻留在字符串常量池中**，而是以特殊的形式存储在运行时常量池中。**只有当这个字符串字面量被调用时，才会对其进行解析，开始为他在字符串常量池中创建对应的 String 实例。**
