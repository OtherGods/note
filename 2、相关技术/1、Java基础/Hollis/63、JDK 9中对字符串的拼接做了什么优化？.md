...
也就说，利用 `invokedynamic` 的特性，将字符串拼接的操作**延迟到运行时**，而不是在编译时固定使用`StringBuilder`。（前面的链接中我们做过反编译，可见 `+` 转成 `StringBuilder` 是编译的时候就确定了的。）
...
`StringBuilder`你不陌生，`MethodHandle` 是啥？他 Java 7 开始引入的特性，它提供了一种灵活且高效的方法来直接操作方法、构造函数和字段的调用。
