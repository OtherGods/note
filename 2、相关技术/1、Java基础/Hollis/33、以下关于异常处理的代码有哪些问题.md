...
# 典型回答
1. `#start`方法不会发生`IOException`，所以不需要`throws`
2. `RuntimeExcption`不需要显式的`throws`
3. `catch`的时候，要先从子类开始`catch`，代码中`catch`的顺序不对
4. 没有关闭流
5. `return`之前的`finally` block是会被执行的
...
千万不能捕获了之后什么也不做。或者只是使用`e.printStacktrace`。
