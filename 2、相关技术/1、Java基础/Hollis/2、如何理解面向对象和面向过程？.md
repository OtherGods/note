...
<font color="red" size=5>面向过程</font>把问题分解成一个一个步骤，<font color="blue" size=5>每个步骤用函数实现，依次调用即可</font>。

我们在进行面向过程编程的时候，不需要考虑那么多，上来先定义一个函数，然后使用各种诸如`if-else`、`for-each`等方式进行代码执行。最典型的用法就是实现一个简单的算法，比如实现冒泡排序。 

<font color="red" size=5>面向对象</font>将问题分解成一个一个步骤，对<font color="blue" size=5>每个步骤进行相应的抽象，形成对象，通过不同对象之间的调用</font>，组合解决问题。

就是说，在进行 **==面向对象编程的时候，要把属性、行为等封装成对象，然后基于这些对象及对象的能力进行业务逻辑的实现==**。比如想要造一辆车，上来要先把车的各种属性定义出来，然后抽象成一个Car类。
...
定义：
1. <font color="red" size=5>通过<font color="blue" size=5>访问权限控制</font>，<font color="blue" size=5>隐藏</font>内部数据，外部仅能通过类提供的有限的接口访问、修改内部数据。</font>
2. 将对象的**数据（属性）** 和 **行为（方法）** 绑定在一起，并**通过访问控制符隐藏内部实现细节，仅暴露必要的接口给外部**。

封装就是把现实世界中的客观事物抽象成一个Java类，然后在类中存放属性和方法。如封装一个汽车类，其中包含了发动机、轮胎 、底盘等属性，并且有启动、前进等方法。

### 继承

定义：<font color="red" size=5>一个类（子类）可以<font color="blue" size=5>复用</font>另一个类（父类）的属性和方法，并且可以<font color="blue" size=5>扩展</font>或<font color="blue" size=5>重写</font>父类的功能</font>
...
定义：<font color="red" size=5>同一个接口/方法调用，在<font color="blue" size=5>不同对象上表现出不同行为</font>的能力</font>
三要素：
1. <font color="blue" size=5>继承</font>
2. <font color="blue" size=5>方法重写</font>（Override）
3. <font color="blue" size=5>父类引用指向子类对象</font>

多态是指在父类中定义的方法被子类继承之后，可以通过重写，使得父类和子类具有不同的实现，这使得<font color="red" size=5>同一个方法在父类及其各个子类中具有不同含义。</font>

[5、如何理解Java中的多态？](2、相关技术/1、Java基础/Hollis/5、如何理解Java中的多态？.md)

### 总结

- **封装**：隐藏实现细节，对外暴露接口。→ Java 用访问修饰符实现。
- **继承**：复用父类属性/方法，支持扩展。→ Java 用 `extends` 实现。
- **多态**：同一接口，不同对象行为不同。→ Java 依赖继承+方法重写。

## 继承和实现

在Java中：
1. 接口可以继承多个接口
2. 抽象类可以实现多个接口、抽象类也可以继承一个抽象类或一个普通类
3. 普通类可以实现多个接口、普通类也可以继承一个抽象类或一个普通类

Java支持多实现，但是只支持单继承。即一个类可以实现多个接口，但是不能继承多个类。

### 为什么Java不支持多继承？

[3、为什么Java不支持多继承？](2、相关技术/1、Java基础/Hollis/3、为什么Java不支持多继承？.md)
