...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508161540111.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508161541979.png)
...
当我们基于`User`类的一个对象user1拷贝出一个新的对象user2的时候，不管怎么样，user1和user2都是两个不同的对象，他们的地址也不会一样。但是其中的成员变量`Address`的话可能就因为深浅拷贝的不同而呈现不同的现象了。
...
即，我们`BeanUtils.copyProperties`拷贝出来的newUser是一个新的对象，但是，其中的address对象和原来的user中的address对象是同一个对象。
...
这种方式就能实现深拷贝，但是问题是如果我们在`User`中有很多个对象，那么`clone`方法就写的很长，而且如果后面有修改，在`User`中新增属性，这个地方也要改。
...
除此之外，还可以使用Apache Commons Lang中提供的`SerializationUtils`工具实现。

我们需要修改下上面的User和Address类，使他们实现`Serializable`接口，否则是无法进行序列化的。
```java
