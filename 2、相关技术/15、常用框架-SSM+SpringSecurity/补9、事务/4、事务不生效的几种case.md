...
// 在这个方法中调用带有事务注解的方法，当抛出异常的时候事务不会滚是因为这里调用的是目标对象中的方法，而不是代理类中的方法（在代理类方法testCompileException2中：先关闭自动提交，之后会判断这个方法是否执行成功，如果执行成功则提交事务，否则回滚事务【这个过程是我猜测的】）。
public boolean testCall(int id) throws Exception {  
    return testCompileException2(id);  
}
```

上面两个方法，直接调用`testCompleException2`方法，事务正常操作；通过调用`testCall`间接访问，在不生效
## 2.3、私有方法

这种场景和2.2也是同一种原因，动态代理设计模式生成的动态代理类和目标类之前的关系我认为类似装饰器模式，在动态代理类不一定能发现目标类中的非public方法（访问权限修饰符），所以目标类中的私有方法有事注解务修饰时即使异常也不会回滚。
...
转载自：[【DB系列】事务不生效的几种case](https://spring.hhui.top/spring-blog/2020/02/03/200203-SpringBoot%E7%B3%BB%E5%88%97%E6%95%99%E7%A8%8B%E4%B9%8B%E4%BA%8B%E5%8A%A1%E4%B8%8D%E7%94%9F%E6%95%88%E7%9A%84%E5%87%A0%E7%A7%8Dcase/)
