...
对照：[4、事务不生效的几种case](2、相关技术/15、常用框架-SSM+SpringSecurity/补9、事务/4、事务不生效的几种case.md)
## 概述
...
这种场景和 同一个类中的方法相互调用 也是同一种原因，动态代理设计模式生成的动态代理类和目标类之前的关系我认为类似装饰器模式，在动态代理类不一定能发现目标类中的非public方法（访问权限修饰符），所以目标类中的私有方法有事注解务修饰时即使异常也不会回滚。
...
- `NESTED` 如果当前上下文中存在事务，则嵌套事务执行（内部事务不影响外部事务，外部事务影响内部事务），如果不存在事务，则新建事务。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408222240092.png)

### 11、通过反射调用其他类的方法

这种情况和 [4. 同一个类中的方法相互调用](2、相关技术/15、常用框架-SSM+SpringSecurity/补9、事务/我认真总结并分析了Spring事务失效的十种常见场景.md#4.%20同一个类中的方法相互调用) 场景一样，都是没有走动态代理
...
{  
   ...
   // 类之间方法直接调用，走事务，数据插入失败
   //User2 user2=new User2();  
   //user2.setName("李四");  
   //user2Service.addRequiresNewException(user2);   

   // 类之间方法通过反射调用，不走事务，数据成功插入
   Class u = Class.forName("org.transaction.test.local_transaction.mybatis.service.impl.User2ServiceImpl");
   Object o = u.getDeclaredConstructor().newInstance();  
   User2 user3=new User2();  
   user3.setName("wangwu");  
   Method m1Method = u.getMethod("addRequiresNewException", User2.class, User2Mapper.class);  
   m1Method.invoke(o, user3, user2Mapper);  
}
```
...
转载自：[我认真总结并分析了Spring事务失效的十种常见场景](https://juejin.cn/post/7198894062534475834)
