...
所谓**作用域**，其实就是说这个东**西哪个范围内可以被使用**。如我们定义类的成员变量的时候使用的`public`、`private`等这些也是作用域的概念。

Spring的Bean的作用域，描述的就是这个Bean在哪个范围内可以被使用。不同的作用域决定了了 `Bean的创建`、`管理`和`销毁`的方式。
...
1. 单例（Singleton）：
	- **默认**作用域。
	- 对于**每个 Spring IoC 容器，只创建一个 Bean** 实例。
	- 适用于**全局共享**的状态。
2. 原型（Prototype）：
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507240903590.png)

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507240906709.png)

# 扩展知识

## 作用域与循环依赖

<font color="red" size=5>Spring在解决循环依赖时，只解决了单例作用域的，别的作用域没有解决</font>：
