#Spring2-6之后不支持，之前默认支持 

# 典型回答

默认支不支持要看版本，在SpringBoot 2.6以前，是默认支持的，但是在 SpringBoot 2.6 开始，默认已经不开启对循环依赖的支持了。

在SpringBoot 2.6及以后版本中，如果代码中出现Spring的Bean的循环依赖，启动会报错，如以下是我的[数藏项目](https://www.yuque.com/hollis666/io9xi1/dgolk0cckpb94sia)中关于出现循环依赖时的报错：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507272215774.png)
...
1. 在配置文件中加入`spring.main.allow-circular-references=true`(**解决属性注入和setter注入**)
2. 用 `@Lazy` 注解（**解决构造注入**），在 `@Autowired` 地方增加即可
