#低版本的Spring会自动处理字段和setter方法的循环依赖（高版本不再自动处理这种循环依赖了），会导致循环依赖被隐藏，使用构造注入能让这种以来暴露出来 
#可能会出现空指针异常，因为属性的注入是发生在初始化过程中，在类加载或实例化过程中使用以字段注入方式注入的对象就可能报空指针 
#Autowire基于Spring，进行单元测试时需要启动Spring，太耗时 
#单一职责 
...
[7、Autowired和Resource的关系？](2、相关技术/15、常用框架-SSM+SpringSecurity/Hollis/Spring/7、Autowired和Resource的关系？.md)
