...
`SimpleAsyncTaskExecutor`这玩意坑很大，其实他<font color="blue" size=5>并不是真的线程池，它是不会重用线程的，每次调用都会创建一个新的线程，也没有最大线程数设置。并发大的时候会产生严重的性能问题。</font>
...
例子用的是这篇的改造的：
[23、在Spring中如何使用Spring Event做事件驱动](2、相关技术/15、常用框架-SSM+SpringSecurity/Hollis/Spring/23、在Spring中如何使用Spring%20Event做事件驱动.md)
...
并且把他声明为`@Configuration`，然后也可以把`Application.java`中的`@EnableAsync`放到这里。

接下来在使用`@Async`的时候，指定一下即可：
控制并发参考：[基于Redis的分布式锁，解决短信验证码重复发放等问题](9、项目难点&亮点/基于Redis的分布式锁，解决短信验证码重复发放等问题.md)
```java
/**
 * 案件中心内部事件监听器
 *
...
public class RegisterEventListener {
    ...
    public void onApplicationEvent(RegisterSuccessEvent event) {
        ...
        //执行发送欢迎短信的逻辑
        //这里注意要控制好并发，这个例子就不细说了
    }
}
```

在@Async中指定`registerSuccessExecutor`即可。这样在后续执行时，就会用到我们自定义的线程池了。
...
简单总结一下，就是会先尝试获取`TaskExecutor`的实现类，这里如果能且仅能找到唯一一个，那么就用这个，如果找不到，或者找到了多个，那么就会走到catch (NoUniqueBeanDefinitionException ex) 和catch (NoSuchBeanDefinitionException ex)的分支中，这里就是获取beanName为taskExecutor的Bean
