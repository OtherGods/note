#Transactional与Async一起标注在方法上事务会生效，Async对应的注解解析器先执行，也就是先创建线程，再开启事务 
...
1. 如果 `@Transactional` 与 `@Async` 加在同一个方法上，那么事务会生效的。
```java
...
2. **如果 `@Transactional` 方法A调用 `@Async` 方法B，那么A 抛异常了，A 自己会回滚，但是B 不会回滚；B 抛异常了 ， 但是A 也不会回滚。**
```java
import org.springframework.scheduling.annotation.Async;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
