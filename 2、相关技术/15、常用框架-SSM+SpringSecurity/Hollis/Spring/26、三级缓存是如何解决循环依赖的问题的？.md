#解决循环依赖的方式是将对象的实例化与初始化分开，不但要缓存成品对象，还要缓存半成品对象和ObjectFactory
#ObjectFactory是函数式接口，可以通过lambda表达式、方法引用、构造引用延迟创建，如果没有发生循环依赖那么代理对象就延迟到初始化结束时创建，如果发生了循环依赖，那么代理对象就需要在实例化之后初始化之前创建） 
