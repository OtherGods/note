...
**流式输出**指的是**服务器将数据以“流”的方式逐步写入响应体，而不是一次性生成整个结果后再返回**。 比较典型的就是现在大语言模型的对话功能，几乎都是以流式输出的方式的。好处就是不用让用户等很长时间。
...
对照：[2、ResponseBodyEmitter类](2、相关技术/25、源码/Spring框架/1、常用类、接口、方法…….md#2、ResponseBodyEmitter类)
...
只需要接口返回一个`Flux`类型即可实现，非常的简单。这是一种**异步非阻塞**的实现方式。推荐用这种。