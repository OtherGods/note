...
[41、什么是事务消息，为什么需要事务消息？](2、相关技术/21、分布式/Hollis/41、什么是事务消息，为什么需要事务消息？.md)

`RocketMQ`的事务消息是通过`TransactionListener`接口来实现的。

在发送事务消息时，首先向`RocketMQ Broker`发送一条“half消息”（即半消息），**半消息将被存储在`Broker`端的==事务消息日志==中，但是这个==半消息还不能被消费者消费==**。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202411212315516.png)
...
流程图:
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508212025552.png)

# 扩展知识

## 如果一直没收到COMMIT或者ROLLBACK怎么办？(第二次消息发送失败)
...
rocketmq与回查相关的三个重要参数，配置在brocker中：[RocketMQ中的消息回查设置](2、相关技术/23、高并发高可用高性能/MQ/尚硅谷/RocketMQ教程.md#RocketMQ中的消息回查设置)
...
> 我认为作者想说的是为什么上游系统向MQ发送消息要分成两步，而不是上游系统本地执行完事务后，再发消息给MQ；从他下面的回答可以看出第一个半消息就是为了确保MQ可以正常工作，不至于上游系统本地执行了事务操作，但是给MQ发送消息的动作失败，导致不一致；简单说就是要先确保MQ可以正常工作再执行本地事务
...
[29、如何基于MQ实现分布式事务](2、相关技术/21、分布式/Hollis/29、如何基于MQ实现分布式事务.md)
