对比：[5.5、Consumer消息消费重复可能的原因](RocketMQ教程#5.5、Consumer消息消费重复可能的原因)

# 典型回答

`RocketMQ`重复消费，指的是同一个消息重复来了多次，那么通常的情况有很多，常见的有以下几个。

## `Consumer`返回给`Broker`消费失败（常见）
...
[14、RocketMQ和Kafka一样有重平衡的问题吗？](2、相关技术/23、高并发高可用高性能/MQ/Hollis/RocketMQ/14、RocketMQ和Kafka一样有重平衡的问题吗？.md)
