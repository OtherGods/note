...
MQ的消费模式可以一共有3种，分别是推`Push`、拉`Pull`以及5.0中推出的 `POP` 模式。本文主要介绍下 `pull` 和 `push`，`pop`，`pop`参考：[12、介绍下 RocketMQ 5.0中的 pop 模式](2、相关技术/23、高并发高可用高性能/MQ/Hollis/RocketMQ/12、介绍下%20RocketMQ%205.0中的%20pop%20模式.md)。
- `Push`是服务端主动推送消息给客户端，**==长轮询==**
	- **优点**是及时性较好
	- **缺点**如果客户端没有做好流控，一旦服务端推送大量消息到客户端时，就会导致客户端消息堆积甚至崩溃
- `Pull`是客户端需要主动到服务端轮询获取数据，**==轮询==**
	- **优点**是客户端可以依据自己的消费能力进行消费
	- **缺点**是频繁拉取会给服务端造成压力，并且可能会导致消息消费不及时
...
在下面这篇文章中我们介绍过长轮询，其实 **`RocketMQ`的`push`就是通过==长轮询==来实现的**。
[消息队列使用拉模式好还是推模式好？为什么？](2、相关技术/23、高并发高可用高性能/MQ/Hollis/RocketMQ/消息队列使用拉模式好还是推模式好？为什么？.md)
