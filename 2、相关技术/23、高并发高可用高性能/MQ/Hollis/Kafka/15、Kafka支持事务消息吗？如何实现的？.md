# 典型回答
很多人说kafka不支持事务消息，但其实，Kafka支持事务消息，只不过<font color="red">此事务消息非彼事务消息</font>。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202412232331000.png)
...
以下是我在极客时间的《消息队列高手课》中看到过的作者画的一张图，供大家参考：![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202412232333459.png)
...
消费者在读取事务消息时，需要将隔离级别设置为read_committed：
```java
Properties props = new Properties();
props.put("bootstrap.servers", "localhost:9092");
...
> Kafka引入了read_committed隔离级别，消费者在该级别下只能读取已提交的事务消息，确保消费的一致性。消费者会跳过尚未提交的消息或已回滚的消息。
