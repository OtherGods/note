...
# 2、分页优化
参考我整理的这篇：[补13_limit深度分页优化](2、相关技术/4、数据库-MySQL/补13_limit深度分页优化.md)
...
```sql
# 这段sql有问题，order by应该在子查询中，同时给score建索引
-- 子查询查出当前分页的首行id
SELECT
	`score`,
	`name` 
FROM
	`cus_order` 
WHERE
	id >= ( SELECT id FROM `cus_order` LIMIT 1000000, 1 )  
	LIMIT 10;
```

我们先查询出 limit 第一个参数对应的主键值，再根据这个主键值再去过滤并 limit，这样效率会更快一些。不过，这种方法只适用于 id 是正序的。
...
<font color = "red" size = 5>优化方式：子查询、延迟关联</font>
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407281744313.png)
...
FROM
	`cus_order` a,
	( SELECT id FROM `cus_order` LIMIT 1000000, 10 ) b 
WHERE
	a.id = b.id;
```
...
FROM
	`cus_order` a
	INNER JOIN ( SELECT id FROM `cus_order` LIMIT 1000000, 10 ) b ON a.id = b.id 
	LIMIT 10;
```
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407282042063.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407282124558.png)
...
这三种种方式都有各自的优势，根据实际场景选择最合适的才是王道。下面再对这三种方式做一个简单的对比，以供大家实际开发中选择正确的存放时间的数据类型：[9、MySQL日期类型选择建议](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/9、MySQL日期类型选择建议.md)
...
BigDecimal的详细介绍请参考这篇：[BigDecimal 详解](https://javaguide.cn/java/basis/bigdecimal.html)。
...
相关阅读：[数据库主键一定要自增吗？有哪些场景不建议自增？](https://mp.weixin.qq.com/s?__biz=MzkxNTU5MjE0MQ==&mid=2247493125&idx=1&sn=f70832caab520c7285f55340eec3efd7&source=41#wechat_redirect)。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407282228003.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407282230246.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407282240171.png)
...
关于 Explain 的详细介绍，请看这篇文章：[7、MySQL执行计划分析](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/7、MySQL执行计划分析.md) 。另外，再推荐一下阿里的这篇文章：[3、如何使用 MySQL 慢查询日志进行性能优化 - Profiling、mysqldumpslow 实例详解](2、相关技术/4、数据库-MySQL/JavaGuide/补充/3、如何使用%20MySQL%20慢查询日志进行性能优化%20-%20Profiling、mysqldumpslow%20实例详解.md) ，总结的挺不错。
...
## 11.2、被频繁更新的字段应该慎重建立索引

虽然索引能带来查询上的效率，但是维护索引的成本也是不小的。 如果一个字段不被经常查询，反而被经常修改，那么就更不应该在这种字段上建立索引了。

## 11.3、尽可能的考虑建立联合索引而不是单列索引
...
- [10、MySQL隐式转换造成索引失效](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/10、MySQL隐式转换造成索引失效.md)
- ……

参考：[2、MySQl 索引之道](2、相关技术/4、数据库-MySQL/JavaGuide/补充/2、MySQl%20索引之道.md)
...
- MySQL 的 COUNT 语句，竟然都能被面试官虐的这么惨 - Hollis：[[MySQL的COUNT语句]]
- MySQL 性能优化神器 Explain 使用分析：[https://segmentfault.com/a/1190000008131735](https://segmentfault.com/a/1190000008131735)
- 如何使用MySQL慢查询日志进行性能优化：[https://xie.infoq.cn/article/e40754334e18838fe100b5526](https://xie.infoq.cn/article/e40754334e18838fe100b5526)
