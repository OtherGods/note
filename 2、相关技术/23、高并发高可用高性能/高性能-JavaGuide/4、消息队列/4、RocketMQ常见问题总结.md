...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282318501.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282319182.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282321751.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282322966.png)

那么第二步，我们又添加了一个发送邮件，我们就得重新去修改代码，如果我们又加一个需求：用户购买完还需要给他加积分，这个时候我们是不是又得改代码？
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282324570.png)

如果你觉得还行，那么我这个时候不要发邮件这个服务了呢，我是不是又得改代码，又得重启应用？
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282324866.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282328921.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282329055.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292224562.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292229997.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292236442.png)
...
每个主题中都有多个队列(分布在不同的 `Broker`中，如果是集群的话，`Broker`又分布在不同的服务器中)，集群消费模式下，一个消费者集群多台机器共同消费一个 `topic` 的多个队列，**一个队列只会被一个消费者消费（我感觉这里说的范围是在一个消费组内）**。如果某个消费者挂掉，分组内其它消费者会接替挂掉的消费者继续消费。就像上图中 `Consumer1` 和 `Consumer2` 分别对应着两个队列，而 `Consumer3` 是没有队列对应的，所以一般来讲要控制 **消费者组中的消费者个数和主题中队列个数相同** 。

当然也可以消费者个数小于队列个数，只不过不太建议。如下图。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292306491.png)

**每个消费组在每个队列上维护一个消费位置** ，为什么呢？

因为我们刚刚画的仅仅是一个消费者组，我们知道在发布订阅模式中一般会涉及到多个消费者组，而每个消费者组在每个队列中的消费位置都是不同的。如果此时有多个消费者组，那么消息被一个消费者组消费完之后是不会删除的(因为其它消费者组也需要呀)，它仅仅是为每个消费者组维护一个 **消费位移(offset)** ，每次消费者组消费完会返回一个成功的响应，然后队列再把维护的消费位移加一，这样就不会出现刚刚消费过的消息再一次被消费了。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292307741.png)

可能你还有一个问题，**为什么一个主题中需要维护多个队列** ？

答案是 **提高并发能力** 。的确，每个主题中只存在一个队列也是可行的。你想一下，如果每个主题中只存在一个队列，这个队列中也维护着每个消费者组的消费位置，这样也可以做到 **发布订阅模式** 。如下图。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292318851.png)
...
- `Broker`：主要负责消息的存储、投递和查询以及服务高可用保证。说白了就是消息队列服务器嘛，生产者生产消息到 `Broker` ，消费者从 `Broker` 拉取消息并消费。
  ...
  **一个 `Topic` 分布在多个 `Broker`上，一个 `Broker` 可以配置多个 `Topic` ，它们是多对多的关系**。
  ...
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292346921.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408292348267.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408311259351.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408311314865.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408311410024.png)
...
具体实现可以参见这篇文章[RocketMQ 对 pull 和 push 的实现open in new window](http://devedmc.com/archives/1691854198138)。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408311705706.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408311738975.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408312245254.png)
...
## [RocketMQ 如何保证高性能读写](#rocketmq-如何保证高性能读写)

### [传统 IO 方式](#传统-io-方式)

![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408312253376.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408312337947.png)
...
这段Java代码使用`FileChannel`和`MappedByteBuffer`来实现文件的内存映射。内存映射文件是一种高效的文件I/O方式，它允许将文件内容直接映射到内存，从而可以像操作内存中的数据一样操作文件中的数据。这对处理大型文件或需要高效读写操作的场景非常有用。
...
1. `new RandomAccessFile("test.txt", "rw")`: 创建一个指向文件`test.txt`的`RandomAccessFile`对象。`rw`模式表示该文件是以读写模式打开的。
2. `getChannel()`: 从`RandomAccessFile`对象获取一个`FileChannel`对象。`FileChannel`是Java NIO（New Input/Output）中的类，用于高效地读取、写入和操作文件。
```java
MappedByteBuffer mappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, fileChannel.size());
```

1. `fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, fileChannel.size())`: 通过`FileChannel`的`map`方法将文件映射到内存。该方法有三个参数：
	1. `FileChannel.MapMode.READ_WRITE`: 映射模式，这里表示映射后的缓冲区是可读可写的。
	2. 0: 文件的起始位置，从文件的开头开始映射。
	3. fileChannel.size(): 映射的区域大小，这里表示映射整个文件。
2. MappedByteBuffer mappedByteBuffer: map方法返回一个MappedByteBuffer对象，它是一个直接字节缓冲区，映射了文件中的内容。通过这个缓冲区，可以直接对文件进行读写操作。

总结：这段代码的作用是将文件`test.txt`映射到内存中，使得可以通过`MappedByteBuffer`对象直接操作文件内容，而不需要通过传统的文件读写操作。这种方式在处理大文件和需要高效文件操作时非常有用，因为它减少了内核和用户空间之间的数据拷贝，提高了I/O操作的性能。

示例代码：
以下是一个完整的示例代码，演示如何使用`FileChannel`和`MappedByteBuffer`：
```java
...
/**
在这个示例中，我们首先读取文件的内容，然后将文件的内容修改为"Hello, World!"，并将修改后的内容写回文件中
*/
public class MemoryMappedFileExample {
    public static void main(String[] args) {
        try {
            // 创建一个RandomAccessFile对象，打开test.txt文件
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408312349340.png)
...
这段Java代码用于使用`FileChannel`从一个文件中传输数据到一个目标通道。`FileChannel` 是 Java NIO（New I/O）的一部分，提供了一种高效的文件I/O操作方式。让我们逐行解释这段代码：

```java
// 打开一个文件通道，指定文件路径为 ./test.txt，打开方式为写入和创建（如果文件不存在则创建）
FileChannel channel = FileChannel.open(Paths.get("./test.txt"), StandardOpenOption.WRITE, StandardOpenOption.CREATE);

// 调用 transferTo 方法从文件的指定位置 position 开始传输 len 字节的数据到目标通道 target
channel.transferTo(position, len, target);
```

1. **`FileChannel.open(Paths.get("./test.txt"), StandardOpenOption.WRITE, StandardOpenOption.CREATE)`**:
   - `FileChannel.open` 方法打开一个文件通道。
   - `Paths.get("./test.txt")` 指定了文件的路径。
   - `StandardOpenOption.WRITE` 表示以写入模式打开文件。
   - `StandardOpenOption.CREATE` 表示如果文件不存在则创建文件。

2. **`channel.transferTo(position, len, target)`**:
   - `transferTo` 方法用于将文件中的数据传输到目标通道。
   - `position` 表示从文件的哪个位置开始传输数据。
   - `len` 表示要传输的数据长度（字节数）。
   - `target` 是目标通道，必须是一个 `WritableByteChannel`（如另一个 `FileChannel` 或 `SocketChannel`）。

示例：
假设我们有一个文件 `test.txt`，我们希望将这个文件的部分数据传输到另一个文件 `output.txt` 中，可以这样做：
...
public class FileTransferExample {
    public static void main(String[] args) {
        try (FileChannel sourceChannel = FileChannel.open(Paths.get("./test.txt"), StandardOpenOption.READ);
             FileChannel targetChannel = FileChannel.open(Paths.get("./output.txt"), StandardOpenOption.WRITE, StandardOpenOption.CREATE)) {

            long position = 0; // 从文件开头开始传输
            long len = sourceChannel.size(); // 传输整个文件
...
在这个示例中：
- 打开 `test.txt` 文件作为源文件通道，以只读模式打开。
- 打开 `output.txt` 文件作为目标文件通道，以写入和创建模式打开。
- 使用 `transferTo` 方法从 `test.txt` 文件开头开始传输整个文件的内容到 `output.txt` 文件。

总结：这段代码通过 `FileChannel` 的 `transferTo` 方法，能够高效地将数据从一个文件传输到另一个文件或者其他可写通道。相比传统的文件复制方法，这种方式在底层使用了操作系统的零拷贝机制，可以大大提高数据传输的效率。

#### 对比mmap和sendfile

在如上两段代码中，发现sendfile并没有文件的读写操作，而是直接将文件的数据传输到 target 目标缓冲区，也就是说，sendfile 是无法知道文件的具体的数据的；但是 mmap 不一样，他是可以修改内核缓冲区的数据的。假设如果需要对文件的内容进行修改之后再传输，只有 mmap 可以满足。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409010015871.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409010024872.png)
...
而在 `RocketMQ` 中采用了 `Dledger` 解决这个问题。他要求在写入消息的时候，要求**至少消息复制到半数以上的节点之后**，才给客⼾端返回写⼊成功，并且它是支持通过选举来动态切换主节点的。这里我就不展开说明了，读者可以自己去了解。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409010038812.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202409010041407.png)
...
著作权归JavaGuide(javaguide.cn)所有 基于MIT协议 原文链接：[https://javaguide.cn/high-performance/message-queue/rocketmq-questions.html](https://javaguide.cn/high-performance/message-queue/rocketmq-questions.html)
...
转载自：[https://javaguide.cn/high-performance/message-queue/rocketmq-questions.html](https://javaguide.cn/high-performance/message-queue/rocketmq-questions.html)
