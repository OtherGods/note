...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282250250.png)

## [Disruptor 是什么？](#disruptor-是什么)

Disruptor 是一个开源的高性能内存队列，诞生初衷是为了解决内存队列的性能和内存安全问题，由英国外汇交易公司 LMAX 开发。

根据 Disruptor 官方介绍，基于 Disruptor 开发的系统 LMAX（新的零售金融交易平台），单线程就能支撑每秒 600 万订单。Martin Fowler 在 2011 年写的一篇文章 [The LMAX Architectureopen in new window](https://martinfowler.com/articles/lmax.html) 中专门介绍过这个 LMAX 系统的架构，感兴趣的可以看看这篇文章。。

LMAX 公司 2010 年在 QCon 演讲后，Disruptor 获得了业界关注，并获得了 2011 年的 Oracle 官方的 Duke's Choice Awards(Duke 选择大奖)。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282252353.png)

> “Duke 选择大奖”旨在表彰过去一年里全球个人或公司开发的、最具影响力的 Java 技术应用，由甲骨文公司主办。含金量非常高！

我专门找到了 Oracle 官方当年颁布获得 Duke's Choice Awards 项目的那篇文章（文章地址：[https://blogs.oracle.com/java/post/and-the-winners-arethe-dukes-choice-award）](https://blogs.oracle.com/java/post/and-the-winners-arethe-dukes-choice-award%EF%BC%89) 。从文中可以看出，同年获得此大奖荣誉的还有大名鼎鼎的 Netty、JRebel 等项目。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282252559.png)

Disruptor 提供的功能优点类似于 Kafka、RocketMQ 这类分布式队列，不过，其作为范围是 JVM(内存)。

- Github 地址：[https://github.com/LMAX-Exchange/disruptoropen in new window](https://github.com/LMAX-Exchange/disruptor)
- 官方教程： [https://lmax-exchange.github.io/disruptor/user-guide/index.htmlopen in new window](https://lmax-exchange.github.io/disruptor/user-guide/index.html)

关于如何在 Spring Boot 项目中使用 Disruptor，可以看这篇文章：[Spring Boot + Disruptor 实战入门open in new window](https://mp.weixin.qq.com/s/0iG5brK3bYF0BgSjX4jRiA) 。

## [为什么要用 Disruptor？](#为什么要用-disruptor)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282257752.png)

Disruptor 真的很快，关于它为什么这么快这个问题，会在后文介绍到。

此外，Disruptor 还提供了丰富的扩展功能比如支持批量操作、支持多种等待策略。

## [Kafka 和 Disruptor 什么区别？](#kafka-和-disruptor-什么区别)

- **Kafka**：分布式消息队列，一般用在系统或者服务之间的消息传递，还可以被用作流式处理平台。
- **Disruptor**：内存级别的消息队列，一般用在系统内部中线程间的消息传递。

## [哪些组件用到了 Disruptor？](#哪些组件用到了-disruptor)
...
## [Disruptor 核心概念有哪些？](#disruptor-核心概念有哪些)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282305589.png)

## [Disruptor 等待策略有哪些？](https://javaguide.cn/high-performance/message-queue/disruptor-questions.html#disruptor-%E7%AD%89%E5%BE%85%E7%AD%96%E7%95%A5%E6%9C%89%E5%93%AA%E4%BA%9B)

**等待策略（WaitStrategy）** 决定了没有事件可以消费的时候，事件消费者如何等待新事件的到来。

常见的等待策略有下面这些：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282306842.png)
Disruptor 等待策略
...
关于 Disruptor 高性能队列原理的详细介绍，可以查看这篇文章：[Disruptor 高性能队列原理浅析open in new window](https://qin.news/disruptor/) （参考了美团技术团队的[高性能队列——Disruptoropen in new window](https://tech.meituan.com/2016/11/18/disruptor.html)这篇文章）。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408282311373.png)
...
- Disruptor 高性能之道-等待策略：<[http://wuwenliang.net/2022/02/28/Disruptoropen in new window](http://wuwenliang.net/2022/02/28/Disruptor) 高性能之道-等待策略/>
- 《Java 并发编程实战》- 40 | 案例分析（三）：高性能队列 Disruptor：[https://time.geekbang.org/column/article/98134open in new window](https://time.geekbang.org/column/article/98134)

---

转载自：[https://javaguide.cn/high-performance/message-queue/disruptor-questions.html](https://javaguide.cn/high-performance/message-queue/disruptor-questions.html)
