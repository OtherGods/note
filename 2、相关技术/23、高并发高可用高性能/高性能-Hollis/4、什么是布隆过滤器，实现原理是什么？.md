...
它的基本原理是利用多个哈希函数，将一个元素映射成多个位，然后将这些位设置为 1。当查询一个元素时，如果这些位都被设置为 1，则认为元素可能存在于集合中，否则**肯定**不存在。

所以，布隆过滤器**可以准确的判断一个元素是否<font color="red" size=5>一定不存在</font>**，但是**因为哈希冲突的存在，所以他没办法判断一个元素一定存在，只能判断可能存在**。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506021141003.png)

所以，布隆过滤器是存在误判的可能的，也就是当一个不存在的Hero元素，经过hash1、hash2和hash3之后，刚好和其他的值的哈希结果冲突了。那么就会被误判为存在，但是其实他并不存在。
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202506021145336.png)

想要降低这种误判的概率，主要的办法就是**降低哈希冲突的概率**及**引入更多的哈希算法**。
...
3. **查询元素是否存在于布隆过滤器中**
<font color="red" size=5>要查询一个元素是否存在于布隆过滤器中，需要将该元素通过多个哈希函数生成多个索引值，并判断这些索引值对应的位是否都被设置为 1。如果这些位都被设置为 1，则认为元素可能存在于集合中，否则肯定不存在。</font>

布隆过滤器的主要优点是可以快速判断一个元素是否属于某个集合 **【我认为应该是快速判断元素是否不存在】** ，并且可以在空间和时间上实现较高的效率。但是，它也存在一些缺点，例如：
