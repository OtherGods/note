来自掘金：[MySQL 为什么采用 B+树作为索引？5年经验程序员回答让我悟了](https://juejin.cn/post/7081065180301361183)
...
如果纯粹的猜测MySQL数据库索引为什么使用B+树？那么围绕这个问题的回答通常一定是围绕B+树本身是什么，有什么优势这两点去解释这个问题。

（这不是我开始这么去想的，看了很多文章都是从这一维度问答，这些回答让我失望啊。直到那天问了坐在我旁边那个整天摸鱼的5年程序员；他慵懒的回答：你想为什么是使用的是树结构呢？ 咦，听到这回答，一下打开了我的思绪，有点意思！）

先抛开B+树是什么，有什么优势，这些先入为主的答案。

（我并不想要这个往我脑袋硬塞的硬邦邦的答案。）
...
你可能对时间单位没什么概念，可以看 1毫秒能慢上纳秒几万倍。

![image-20220929163518980](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\image-20220929163518980.png)

索引虽然存储在磁盘上，但使用索引查找数据时，可以从磁盘先读取索引放到内存中，再通过索引从磁盘找到数据；再然后将磁盘读取到的数据也放到内存里。

索引就让磁盘和内存强强联手，趁机搭上了内存的车，感受了一把纳秒级别速度的推背感。

![在这里插入图片描述](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/802eefd7aa384d618e7bbb4437a44c39~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![由下标访问到哈希表中存的`Key、Value`](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\由下标访问到哈希表中存的`Key、Value`.png)
...
![匹配出正确的 `Key` 就可以](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\匹配出正确的 `Key` 就可以.png)
...
![时间复杂度是 `O(n)`](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\时间复杂度是 `O(n)`.png)

如下图所示，跳表是在链表基础上加了索引层。可以起到支持区间查询的作用。

![起到支持区间查询的作用](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\起到支持区间查询的作用.png)
...
![跳表这家伙其实就是在告诉链表从什么地方开始找比较快](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\跳表这家伙其实就是在告诉链表从什么地方开始找比较快.png)

看到这，跳表似乎也很适合用来作为索引的数据结构。但是不要忘了还有首个条件没满足，就是 **"要尽少在磁盘做 I/O 操作。"** 而跳表显然没能满足这一条件，跳表在随数据量增多的情况，索引层也会随着增高，相应的就会增加读取IO的次数，从而影响性能。
...
![树的末端没有节点的称为 `叶子节点`](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\树的末端没有节点的称为 `叶子节点`.Png)

从树的层级关系可以分为上下级和同级节点，如下图，`D、E`是B节点的子节点，那么`B` 节点就是它们的父节点，跟`B`节点在同一层级的`C`节点是`B`节点的兄弟节点。

同时树的最大层级数，称为树的高度（深度），图下的树高度是**3**。

![图下的树高度是3](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\图下的树高度是3.png)
...
![所有子节点在同一层级](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\所有子节点在同一层级.png)

**完全二叉树**

完全二叉树的定义是如果这颗树的所有节点和同深度的满二叉树的的节点位置相同则这二叉树是完全二叉树。如下图。

![如下图](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\如下图.png)
...
![如下图所示](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\如下图所示.png)

所以说，二叉查找树不同于普通二叉查找树，是将小于根节点的元素放在左子树，而右子树正好相反是放大于根节点的元素。（说白了就是根节点是左子树和右子树的中位数，左边放小于中位数的，右边放大于中位数，这不就是二分查找算法的奥义）

![这不就是二分查找算法的奥义](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\这不就是二分查找算法的奥义.png)
...
![查询数据是的时间复杂度就会是`O(n)`](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\查询数据是的时间复杂度就会是`O(n)`.png)
...
![每次插入的元素数据都会维持自平衡](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\每次插入的元素数据都会维持自平衡.png)

如下图所示，普通非二叉树和平衡二叉树的对比。

![普通非二叉树和平衡二叉树的对比](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\普通非二叉树和平衡二叉树的对比.png)
...
![如下图是一颗3阶的B树](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\如下图是一颗3阶的B树.png)
...
![如下图B+树的结构](D:\Tyora\AssociatedPicturesInTheArticles\补8-MySQL为什么采用B+树索引\如下图B+树的结构.png)
...
我是一颗剽悍的种子，**怕什么真理无穷，进一寸，有进一寸的欢喜**。欢迎各位的：**关注**、**点赞**、**收藏**和**评论**，我们下回见！
...
1. 减少磁盘IO的次数
2. 从查询的角度出来，等值查询，范围查询中选出最优的数据结构。
   等值查询：哈希、跳表，不适合范围查询。
   范围查询：树结构，但是会带来磁盘IO的情况，也会造成树过高的问题，所以衍生出B树，而B树又因为范围查询的情况下， 会一直到根节点再到叶子节点这样查询。所以延伸出了B+树，解决范围查询的情况。



作者：一颗剽悍的种子
链接：https://juejin.cn/post/7081065180301361183
来源：稀土掘金
