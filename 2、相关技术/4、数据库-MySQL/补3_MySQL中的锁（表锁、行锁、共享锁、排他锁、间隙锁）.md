原文连接：https://blog.csdn.net/soonfly/article/details/70238902

本文参考：
http://mysqlpub.com/thread-5383-1-1.html
http://blog.csdn.net/c466254931/article/details/53463596
有很多是转载合并过来。
...
![](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419113843041)

**MyISAM存储引擎的读锁阻塞写例子:**
一个session使用LOCK TABLE命令给表film_text加了读锁，这个session可以查询锁定表中的记录，但更新或访问其他表都会提示错误；同时，另外一个session可以查询表中的记录，但更新就会出现锁等待。

![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170517091145402)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419152812783)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419164638753)
...
- **共享锁（s）：**又称读锁。允许一个事务去读一行，阻止其他事务获得相同数据集的排他锁。若事务T对数据对象A加上S锁，则事务T可以读A但不能修改A（我测试的时候，在得到了共享锁的前提下可以修改A），其他事务只能再对A加S锁，而不能加X锁，直到T释放A上的S锁。这保证了其他事务可以读A，但在T释放A上的S锁之前不能对A做任何修改。
...
mysql InnoDB引擎默认的修改数据语句：<u>***update,delete,insert都会自动给涉及到的数据加上排他锁，select语句默认不会加任何锁类型***</u>，如果加排他锁可以使用select …for update语句，加共享锁可以使用select … lock in share mode语句。**<u>*所以加过排他锁的数据行在其他事务种是不能修改数据的，也不能通过for update和lock in share mode锁的方式查询数据，但可以直接通过select …from…查询数据，因为普通查询没有任何锁机制*</u>**。
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419165851422)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419172214144)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170419172837173)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170420090614144)

InnoDB存储引擎使用相同索引键的阻塞例子
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170420090933646)
...
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170714163651485)
...
InnoDB存储引擎的间隙锁阻塞例子
![这里写图片描述](D:\Tyora\AssociatedPicturesInTheArticles\MySQL中的锁（表锁、行锁、共享锁、排他锁、间隙锁）\20170420095938233)
