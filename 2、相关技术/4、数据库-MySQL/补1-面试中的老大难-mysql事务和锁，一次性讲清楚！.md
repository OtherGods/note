对比：
[3、MySQL事务隔离级别详解](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/3、MySQL事务隔离级别详解.md)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072305486.png)
...
在事务并发执行时，如果不进行任何控制，可能会出现以下4类问题：（又可以分为两大类：写写并发、读写并发）

- 脏写（Dirty Write、写写并发） **脏写是指一个事务修改了其它事务未提交的数据**。
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072306801.png)
...
- 脏读（Dirty Read、读写并发） **脏读是指一个事务读到了其它事务未提交的数据**。 
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072307280.png)
...
- 不可重复读（Non-Repeatable Read、读写并发） **不可重复读指的是在一个事务执行过程中，读取到其它事务已提交的数据，导致两次读取的结果不一致**
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072308662.png)
...
- 幻读（Phantom、读写并发） **幻读是指的是在一个事务执行过程中，读取到了其他事务新插入数据，导致两次读取的结果不一致**。
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072309963.png)
...
**脏写的问题太严重了，任何隔离级别都必须避免**。
其它无论是**脏读，不可重复读，还是幻读**，它们<font color = "red" size = 5>都属于数据库的读一致性的问题</font>，**都是在一个事务里面前后两次读取出现了不一致的情况**。
...
MVCC(Multi Version Concurrency Control)，中文名是多版本并发控制，简单来说就是<font color = "red" size = 5>通过维护数据历史版本，从而<font color = "blue" size = 6>解决并发访问情况下的读一致性问题</font>。</font>
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072315333.png)

假设之后两个事务`id`分别为`100`、`200`的事务对这条记录进行`UPDATE`操作，操作流程如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072315982.png)

由于每次变动都会先把`undo`日志记录下来，并用`roll_pointer`指向`undo`日志地址。因此可以认为，**对该条记录的修改日志串联起来就形成了一个`版本链`，版本链的头节点就是当前记录最新的值**。如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072317733.png)
...
- **记录锁(Record Locks)** 所谓记录，就是指聚簇索引中真实存放的数据，比如上面的1、4、7、10都是记录。
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072351904.png)
...
- **间隙锁(Gap Locks)** 间隙指的是两个记录之间逻辑上尚未填入数据的部分，比如上述的(1,4)、(4,7)等。
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072352048.png)
...
- **临键锁(Next-Key Locks)** 
  不同版本范围不同？参考：[MySQL next-key lock 加锁范围是什么？]()
  这个章节中有我整理的临键锁解决幻读的实例：[2.5.1、解决幻读的三种方法](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/3、MySQL事务隔离级别详解.md#2.5.1、解决幻读的三种方法)
  临键指的是间隙加上它右边的记录组成的左开右闭区间。比如上述的(1,4]、(4,7]等。
  ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202408072353498.png)
  
  <font color = "red">临键锁就是记录锁(Record Locks)和间隙锁(Gap Locks)的结合，即除了锁住记录本身，还要再锁住索引之间的间隙</font>。**当我们使用范围查询，并且命中了部分`record`记录，此时锁住的就是临键区间。注意，临键锁锁住的区间会包含最后一个record的右边的临键区间。例如`select * from t where id > 5 and id <= 7 for update;`会锁住(4,7]、(7,+∞)【这里说的有问题，我实际试了一次，只会锁住（4,10）】。mysql默认行锁类型就是`临键锁(Next-Key Locks)`。当使用唯一性索引，等值查询匹配到一条记录的时候，临键锁(Next-Key Locks)会退化成记录锁；没有匹配到任何记录的时候，退化成间隙锁**。

<font color = "red" size = 5>`间隙锁(Gap Locks)`和`临键锁(Next-Key Locks)`都是用来解决幻读问题的，在`已提交读（READ COMMITTED）`隔离级别下，`间隙锁(Gap Locks)`和`临键锁(Next-Key Locks)`都会失效！</font>

作者：夜尽天明_
链接：[https://juejin.cn/post/6855129007336521741](https://juejin.cn/post/6855129007336521741)
来源：稀土掘金
