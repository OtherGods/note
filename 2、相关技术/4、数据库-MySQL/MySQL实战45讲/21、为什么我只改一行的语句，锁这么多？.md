![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507172156039.png)
...
3. 优化 1：索引上的 **`等值查询`，给唯一索引加锁的时候**，**`next-key lock` 退化为行锁**。
4. 优化 2：索引上的 **`等值查询`，向右遍历时且最后一个值不满足等值条件的时候**，**`next-key lock` 退化为间隙锁**。
5. 一个 bug：唯一索引上的范围查询会访问到不满足条件的第一个值为止。
> 热心网友总结：
> 1. 查询过程中访问到的对象才会加锁，而加锁的基本单位是next-key lock（前开后闭）；
> 2. <font color="blue" size=5>等值查询</font>上MySQL的优化：
> 	1. 如果是<font size=5 color="red">唯一索引</font>，next-key lock会**退化为行锁**；
> 		1. 查不存在的索引值，next-key lock会**退化为间隙锁**；
> 	2. <font size=5 color="red">不是唯一索引</font>，**需要访问到第一个不满足条件的值**，此时next-key lock会**退化为间隙锁**；【会锁住索引前/后间隙】
> 3. <font color="blue" size=5>范围查询</font>：<font color="red" size=5>无论是否是唯一索引</font>，**范围查询都需要访问到不满足条件的第一个值为止**，<font color="red" size=5>不会退化</font>；
...
第一个例子是关于**等值条件**操作间隙：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507172203565.png)
...
第二个例子是关于**覆盖索引上的锁**：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507172213323.png)
...
这个例子说明，<font color="red" size=5>锁是加在索引上的</font>；同时，它给我们的指导是，如果你要用 lock in share mode 来给行加读锁避免数据被更新的话，就必须得绕过覆盖索引的优化，在查询字段中加入索引中不存在的字段。比如，将 session A 的查询语句改成 `select d from t where c=5 lock in share mode`，我执行后发现session B和C 都阻塞住了，我感觉估计是因为回表导致主键索引也被锁住了。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181113746.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181138443.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181448501.png)
...
新插入的这一行 c=10，也就是说现在表里有两个 c=10 的行。那么，这时候索引 c 上的间隙是什么状态了呢？你要知道，由于**非唯一索引上包含主键的值，所以是不可能存在“相同”的两行**的。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181452098.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181454092.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181500476.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181501233.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181502246.png)

可以看到，(c=10,id=30）之后的这个间隙并没有在加锁范围里，因此 insert 语句插入 c=12 是可以执行成功的。

这个例子对我们实践的指导意义就是，**在删除数据的时候尽量加 limit**。这样<font color="red" size=5>不仅可以控制删除数据的条数，让操作更安全，还可以减小加锁的范围</font>。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181504406.png)
...
1. 先是加 (5,10) 的**间隙锁**，加锁成功；
2. 然后加 c=10 的**行锁**，这时候才被锁住的【如果这里不阻塞住的话，正常还会有第三步给（10，15）加间隙锁】

也就是说，我们在分析加锁规则的时候可以用 next-key lock 来分析。但是要知道，具体执行的时候，是要分成间隙锁和行锁两段来执行的。

# 小结

这里我再次说明一下，我们上面的所有案例都是在**可重复读隔离级别** (repeatable-read) 下验证的。同时，可重复读隔离级别遵守两阶段锁协议，**所有加锁的资源，都是在事务提交或者回滚的时候才释放的**。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507181519109.png)
...
极客时间版权所有: https://time.geekbang.org/column/article/75659?utm_campaign=geektime_search&utm_content=geektime_search&utm_medium=u_nav_web&utm_source=u_nav_web&utm_term=u_nav_web