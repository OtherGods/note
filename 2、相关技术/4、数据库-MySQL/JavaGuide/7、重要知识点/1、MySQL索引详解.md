...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222018826.png)

但是！哈希算法有个 **Hash 冲突** 问题，也就是说多个不同的 key 最后得到的 index 相同。通常情况下，我们常用的解决办法是 **链地址法**。链地址法就是将哈希冲突数据存放在链表中。就比如 JDK1.8 之前 `HashMap` 就是通过链地址法来解决哈希冲突的。不过，JDK1.8 以后`HashMap`为了减少链表过长的时候搜索时间过长引入了红黑树。

![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407212251752.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407212320092.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407212321267.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222017626.png)
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407212340459.png)
...
二级索引:
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407212339741.png)
...
在 MySQL 中，InnoDB 引擎的表的 `.ibd`文件就包含了该表的索引和数据，对于 InnoDB 引擎表来说，该表的索引(B+树)的每个非叶子节点存储索引，叶子节点存储索引和索引对应的数据（**该主键对应在表中的对应记录**）。
...
这是 MySQL 的表的文件截图:
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222142084.png)

聚簇索引和非聚簇索引:
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222140120.png)

#### 7.2.3、非聚簇索引一定回表查询吗(覆盖索引)?
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222219925.png)
覆盖索引
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222231504.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407222232140.png)

通过 `Extra` 这一列的 `Using index` ，说明这条 SQL 语句成功使用了覆盖索引。

关于 `EXPLAIN` 命令的详细介绍请看：[7、MySQL执行计划分析](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/7、MySQL执行计划分析.md) 这篇文章。
...
最左匹配原则会一直向右匹配，直到遇到范围查询（如 >、<）为止。对于 >=、<=、BETWEEN 以及前缀匹配 LIKE 的范围查询，不会停止匹配（相关阅读：[1、全网都在说一个错误的结论](2、相关技术/4、数据库-MySQL/JavaGuide/补充/1、全网都在说一个错误的结论.md) ）。
...
![1.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407232342503.png)
...
MySQL 8.0.13 版本引入了索引跳跃扫描（Index Skip Scan，简称 ISS），它可以在某些索引查询场景下提高查询效率。在没有 ISS 之前，不满足最左前缀匹配原则的联合索引查询中会执行全表扫描。而 ISS 允许 MySQL 在某些情况下避免全表扫描，即使查询条件不符合最左前缀。不过，这个功能比较鸡肋， 和 Oracle 中的没法比，MySQL 8.0.31 还报告了一个 bug：[Bug #109145 Using index for skip scan cause incorrect resultopen in new window](https://bugs.mysql.com/bug.php?id=109145)（后续版本已经修复）。个人建议知道有这个东西就好，不需要深究，实际项目也不一定能用上。

## 9、索引下推

**索引下推（Index Condition Pushdown，简称 ICP）** 是 **MySQL 5.6** 版本中提供的一项索引优化功能，它<font color = "red">允许存储引擎在索引遍历过程中，执行部分 WHERE字句的判断条件，直接过滤掉不满足条件的记录，从而减少回表次数，提高查询效率</font>。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407242217801.png)
**Using index condition**：表示查询优化器选择使用了索引条件下推这个特性

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407242217319.png)

再来讲讲索引下推的具体原理，先看下面这张 MySQL 简要架构图。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407242233595.png)
...
> ChatGPT介绍索引下推送
> MySQL的索引下推（Index Condition Pushdown, ICP）是一种优化技术，主要用于提高查询性能。它的基本原理是将部分查询条件下推到存储引擎层，在读取数据之前尽可能多地过滤数据，减少从磁盘读取的数据量。索引下推通常在以下几种情况下会被用到：
> 
> 1. **覆盖索引查询**：如果查询的列都是索引列，MySQL可以只通过索引来满足查询，而不需要访问实际的数据行。索引下推可以帮助在索引扫描过程中应用WHERE条件。
> 
> 2. **复合索引的部分列过滤**：当查询条件中使用了复合索引的一部分列时，索引下推可以在存储引擎层通过这些索引列来过滤数据，而不必将所有数据行都读入内存再进行过滤。
> 
> 3. **查询条件包含非索引列**：当查询条件包含非索引列时，传统方式需要读取所有满足索引条件的行，然后在MySQL服务器层进一步过滤。但通过索引下推，MySQL可以在存储引擎层对索引列进行部分过滤，减少需要返回到服务器层进行进一步处理的数据量。
> 
> 4. **范围查询**：对于范围查询（如`BETWEEN`、`>`, `<`等），索引下推可以应用在索引扫描过程中，减少不必要的数据访问。
> 
> 5. **多表联接查询**：在多表联接查询中，如果其中一个表使用了索引并且索引列被用作联接条件，索引下推可以在联接时应用索引条件，减少联接时需要处理的数据量。
> 
> **示例**：
> 假设有一个表`employees`，包含如下字段：`id`（主键）、`name`、`age`、`department`，并在`age`和`department`字段上创建了复合索引。
> 
> ```sql
> SELECT * FROM employees WHERE age > 30 AND department = 'Sales';
> ```
> 
> 在这种情况下，索引下推可以在存储引擎层使用`age > 30`和`department = 'Sales'`的条件进行过滤，从而减少读取的数据量，提高查询效率。
> 
> 要启用或检查索引下推的使用情况，可以通过设置`optimizer_switch`参数：
> 
> ```sql
> SET optimizer_switch = 'index_condition_pushdown=on';
> ```
> 
> 可以通过执行`EXPLAIN`来查看查询计划中是否使用了索引下推：
> 
> ```sql
> EXPLAIN SELECT * FROM employees WHERE age > 30 AND department = 'Sales';
> ```
> 
> 在查询计划中，如果显示了`Using index condition`，则表示使用了索引下推。
> 
> 总之，索引下推的主要目的是通过将过滤条件尽量下推到存储引擎层，减少不必要的数据访问，从而提升查询性能。
...
- 发生 [10、MySQL隐式转换造成索引失效](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/10、MySQL隐式转换造成索引失效.md)
- ……

推荐阅读这篇文章：[2、MySQl 索引之道](2、相关技术/4、数据库-MySQL/JavaGuide/补充/2、MySQl%20索引之道.md)
...
各个字段的含义如下：
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407242336684.png)

参考：[7、MySQL执行计划分析](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/7、MySQL执行计划分析.md)

---

著作权归JavaGuide(javaguide.cn)所有 基于MIT协议 原文链接：[https://javaguide.cn/database/mysql/mysql-index.html](https://javaguide.cn/database/mysql/mysql-index.html)
