...
数值类型，对一个允许为 `NULL` 的字段进行 `min、max、sum、加减、order by、group by、distinct` 等操作的时候。字段值为非 `NULL` 值时，操作很明确。如果使用 `NULL`， 需要清楚的知道如下规则：
...
转载自：[技术分享 | MySQL默认值选型（是空，还是 NULL）](https://opensource.actionsky.com/20190710-mysql/)