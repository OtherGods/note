...
推荐阅读一下我写的这篇文章：[3、字符集详解](2、相关技术/4、数据库-MySQL/JavaGuide/3、字符集详解.md) 。
...
### 3.2、避免使用 TEXT,BLOB 数据类型，最常见的 TEXT 类型可以存储 64k 的数据
...
相关阅读：[5、MySQL默认值选型（是空，还是 NULL）](2、相关技术/4、数据库-MySQL/JavaGuide/补充/5、MySQL默认值选型（是空，还是%20NULL）.md) 。
...
MySQL 时间类型选择的详细介绍请看这篇：[9、MySQL日期类型选择建议](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/9、MySQL日期类型选择建议.md) 。

### 3.6、同财务相关的金额类数据必须使用 decimal 类型
...
### 4.1、限制每张表上的索引数量,建议单张表索引不超过 5 个
...
### 4.3、禁止给表中的每一列都建立单独的索引

5.6 版本之前，一个 sql 只能使用到一个表中的一个索引，5.6 以后，虽然有了合并索引的优化方式，但是还是远远没有使用一个联合索引的查询方式好。

### 4.4、每个 InnoDB 表必须有个主键
...
### 4.7、避免建立冗余索引和重复索引（增加了查询优化器生成执行计划的时间）

- 重复索引示例：primary key(id)、index(id)、unique index(id)
- 冗余索引示例：index(a,b,c)、index(a,b)、index(a)

### 4.8、对于频繁的查询优先考虑使用覆盖索引
...
### 5.4、禁止使用 SELECT * 必须使用 SELECT <字段列表> 查询
...
详细解读可以看：[10、MySQL隐式转换造成索引失效](2、相关技术/4、数据库-MySQL/JavaGuide/7、重要知识点/10、MySQL隐式转换造成索引失效.md) 这篇文章。

### 5.8、避免使用子查询，可以把子查询优化为 join 操作
...
### 5.11、对应同一列进行 or 判断时，使用 in 代替 or

in 的值不要超过 500 个，in 操作可以更有效的利用索引，or 大多数情况下很少能利用到索引。

### 5.12、禁止使用 order by rand() 进行随机排序

order by rand() 会把表中所有符合条件的数据装载到内存中，然后在内存中对所有数据根据随机生成的值进行排序，并且可能会对每一行都生成一个随机值，如果满足条件的数据集非常大，就会消耗大量的 CPU 和 IO 及内存资源。

推荐在程序中获取一个随机值，然后从数据库中获取数据的方式。

### 5.13、WHERE 从句中禁止对列进行函数转换和计算
...
### 5.14、在明显不会有重复值时使用 UNION ALL 而不是 UNION

- UNION 会把两个结果集的所有数据放到临时表中后再进行去重操作
- UNION ALL 不会再对结果集进行去重操作

### 5.15、拆分复杂的大 SQL 为多个小 SQL

- 大 SQL 逻辑上比较复杂，需要占用大量 CPU 进行计算的 SQL
- MySQL 中，一个 SQL 只能使用一个 CPU 进行计算
- SQL 拆分后可以通过并行执行来提高处理效率

### 5.16、程序连接不同的数据库使用不同的账号，禁止跨库查询
...
## 6、数据库操作行为规范
### 6.1、超 100 万行的批量写 (UPDATE,DELETE,INSERT) 操作,要分批多次进行操作
...
### 6.2、对于大表使用 pt-online-schema-change 修改表结构
...
- [技术同学必会的 MySQL 设计规约，都是惨痛的教训 - 阿里开发者open in new window](https://mp.weixin.qq.com/s/XC8e5iuQtfsrEOERffEZ-Q)
- [聊聊数据库建表的 15 个小技巧open in new window](https://mp.weixin.qq.com/s/NM-aHaW6TXrnO6la6Jfl5A)



转载自：[MySQL高性能优化规范建议总结](https://javaguide.cn/database/mysql/mysql-high-performance-optimization-specification-recommendations.html)
