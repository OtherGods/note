...
- next key lock（临键锁） 主要是范围匹配的场景下，会锁某一个范围区间
- gapLock（间隙锁） 主要用来锁边界
...
`-- 读锁 select * from table lock in share mode; -- 写锁 select * from table for update;`
...
![180323_LOCK2.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d841e03822b~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)

**case2:** 唯一索引+rc级别

- sql2加写锁，如下图的case，就两把锁，一个对应于id unique索引上的id = 10的记录，另一把锁对应于聚簇索引上的[name=’d’,id=10]的记录

![180323_LOCK3.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d841e8da776~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)

**case3:** id非唯一索引+RC

- sql2加写锁，如下图的case，会有四个写锁

![180323_LOCK4.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d841e138eac~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
![180323_LOCK5.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d841dd612e1~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
![180323_LOCK6.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d841f2c6c62~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)

**case8:** 无索引+RR

- 在Repeatable Read隔离级别下，如果进行全表扫描的当前读，那么会锁上表中的所有记录，同时会锁上聚簇索引内的所有GAP，杜绝所有的并发 更新/删除/插入 操作

![180323_LOCK7.jpg](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d84a7edaae7~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
![180323_LOCK8.gif](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d84a810345b~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
`-- 设置会话隔离级别 set session transaction ioslation read commited; -- 查看当前会话隔离级别 select @@tx_isolation; -- 会话1的操作 start transaction; select * from newuser where userId=1; -- 会话2开始操作 start transaction; select * from newuser where userId=1; update newuser set updated=1521786092 where userId=1; select * from newuser where userId=1; commit; -- 再次进入会话1，同样执行上次的sql，对比两次输出结果 select * from newuser where userId=1; -- 注意观察，会话1，前后两次这个sql的输出结果，特别是updated字段 -- 正常情况会如上面的demo图，会发生改变 -- 关闭会话 commit; -- 再次查询 select * from newuser where userId=1;`
...
![180323_LOCK9.gif](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d84a82a8a76~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
`-- 会话1 start transaction; select * from newuser where userId=1 lock in share mode; -- 转入会话2 start transaction; select * from newuser where userId=1; -- 会输出 select * from newuser where userId=1 lock in share mode; -- 会输出 update newuser set updated=1521787137 where userId=1; -- 会挂起 -- 转入会话1 -- 提交, 此时观察会话2的写是否完成 commit; -- 转入会话2 commit;`

实际执行演示:

![180323_LOCK10.gif](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d84a7f1cdd8~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
`-- 会话1 start transaction; select * from newuser where userId=1 for update; -- 转入会话2 start transaction; select * from newuser where userId=1; -- 会输出 select * from newuser where userId=1 lock in share mode; -- 会挂住 -- update newuser set updated=1521787137 where userId=1; -- 会挂住 -- 转入会话1 -- 提交, 此时观察会话2的写是否完成 commit; -- 转入会话2 commit;`

实际执行演示:

![180323_LOCK11.gif](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/3/23/16251d84b5a298c8~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
`start transaction; -- xxx 具体的sql commit;`

## IV. 其他

### 参考

- [深入理解Mysql——锁、事务与并发控制](https://link.juejin.cn?target=http%3A%2F%2Fblog.csdn.net%2Flemon89%2Farticle%2Fdetails%2F51477497 "http://blog.csdn.net/lemon89/article/details/51477497")
- [MySQL 加锁处理分析](https://link.juejin.cn?target=http%3A%2F%2Fhedengcheng.com%2F%3Fp%3D771%23_Toc374698322 "http://hedengcheng.com/?p=771#_Toc374698322")

  

作者：一灰灰  
链接：[https://juejin.cn/post/6844903582295130125 ](https://juejin.cn/post/6844903582295130125 ) 
来源：稀土掘金  
...
转载于：[mysql之锁与事务](https://juejin.cn/user/377887729916126/posts)
