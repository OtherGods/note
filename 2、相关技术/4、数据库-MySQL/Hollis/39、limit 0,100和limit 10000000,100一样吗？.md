...
参照：[68、MySQL的深度分页如何优化](2、相关技术/4、数据库-MySQL/Hollis/68、MySQL的深度分页如何优化.md)
...
[118、limit的原理是什么？](2、相关技术/4、数据库-MySQL/Hollis/118、limit的原理是什么？.md)

所以，`limit 10000000,100`要比`limit 0,100`的性能差的多，因为他要先读取10000100条数据，然后再抛弃前面的10000000条。
[68、MySQL的深度分页如何优化](2、相关技术/4、数据库-MySQL/Hollis/68、MySQL的深度分页如何优化.md)
...
所以，<font color="red" size=5>当order by语句中，有Limit的时候，可能每次的查询结果都不一样</font>。比如有下面两次查询的结果可能就不一样：
```sql
