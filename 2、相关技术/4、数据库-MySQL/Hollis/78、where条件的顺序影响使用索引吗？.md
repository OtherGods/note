...
<font color="red" size=5>因为有<font color="blue" size=5>查询优化器</Font>的存在，字段的先后顺序并不重要</font>，不信的话可以explain看一下上面两个SQL的执行计划，都是可以命中(a,b)的联合索引的。
跳跃索引：[索引跳跃扫描](34、MySQL索引一定遵循最左前缀匹配吗？#索引跳跃扫描)
