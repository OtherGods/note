...
6、MySQL的MyISAM引擎对`COUNT(*)`做了哪些优化？
7、MySQL的InnoDB引擎对`COUNT(*)`做了哪些优化？
8、上面提到的MySQL对`COUNT(*)`做的优化，有一个关键的前提是什么？
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407292320182.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202407292322561.png)
...
最后，就是我们一直还没提到的`COUNT(字段)`，他的查询就比较简单粗暴了，就是进行全表扫描，然后判断指定字段的值是不是为NULL，不为NULL则累加。
...
转载自：[MySQL的COUNT语句，竟然都能被面试官虐的这么惨！？](https://mp.weixin.qq.com/s/IOHvtel2KLNi-Ol4UBivbQ)