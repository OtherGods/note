...
8. **ORDER BY：**
	- 如果查询指定了ORDER BY，现在将<font color="red" size=5>对结果进行排序</font>。注意，如果使用了GROUP BY，**MySQL可能会利用这个排序作为GROUP BY的一部分。**
9. **LIMIT：**
	- 最后，应用LIMIT子句来限制返回的行数。这通常是<font color="red">整个查询过程的最后一步。</font>
