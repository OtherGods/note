...
[51、MySQL主从复制的过程](2、相关技术/4、数据库-MySQL/Hollis/51、MySQL主从复制的过程.md)

在MySQL的主从复制中，我们介绍过MySQL的主从复制的原理，在复制过程中，**主库的binlog会不断地同步到从库**，然后**从库有一个SQL线程不断地拉取并重放这些SQL语句**，那么，一旦日志内容太多的话，*一个线程执行就会有延迟，就会导致主从延迟*。
...
[72、介绍下MySQL 5.7中的组提交](2、相关技术/4、数据库-MySQL/Hollis/72、介绍下MySQL%205.7中的组提交.md)
...
所以，这样 **==Slave就可以用多个SQL线程来并行的执行一个组提交中的多条SQL，从而提升效率，降低主从延迟==**。
...
MySQL 8.0为了解决这个问题，引入了基于WriteSet的并行复制，这种情况下即使主库在串行提交的事务，只要互相不冲突，在备库就可以并行回放。
