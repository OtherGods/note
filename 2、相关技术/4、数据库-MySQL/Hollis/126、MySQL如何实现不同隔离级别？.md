...
| ---------------- | --- | ----- | --- | ---------------------------------- |
| READ UNCOMMITTED | Y   | Y     | Y   | **直接读取最新数据（无版本控制）**                |
| READ COMMITTED   | N   | Y     | Y   | **每次查询生成新 Read View**              |
| REPEATABLE READ  | N   | N     | Y   | **事务开始时生成 Read View + Next-Key 锁** |
| SERIALIZABLE     | N   | N     | N   | **所有读操作加共享锁，写操作加排他锁**              |
[14、InnoDB如何解决脏读、不可重复读和幻读的？](2、相关技术/4、数据库-MySQL/Hollis/14、InnoDB如何解决脏读、不可重复读和幻读的？.md)
