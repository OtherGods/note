#显示定义自增ID #隐示主键ID：row_id #显示自增ID用完报主键冲突 #隐示row_id用完从0开始继续自增  

# 典型回答

我们知道，在MySQL中，自增主键有两种，一种是显式的、一种是隐式的。如果我们在一张表中没有定义主键，那么，MySQL会创建一个**隐藏的主键（row_id）作为主键**。

那么，不管是我们自己定义的自增主键，还是row_id的这个主键，都是一个固定类型的，一般都是**bigint unsigned**，那么既然有固定类型，就有取值范围。那么随着数据量的增长，主键的值会不断增长，那么万一超过了这个范围限制，会怎么样呢？
...
所以，结论是：
- **显示自定义的自增ID**，用完以后下次插入会报**主键冲突**。
- **未定义自增ID主键**，会用row_id，用完以后下一次插入会**覆盖历史数据**。
...
3. **使用UUID作为主键（不推荐）：**
	- 考虑使用 UUID（通用唯一标识符）作为主键。UUID 是128位长，几乎不可能用完。但这会增加存储需求，并可能影响性能。
	  [36、uuid和自增id做主键哪个好，为什么？](2、相关技术/4、数据库-MySQL/Hollis/36、uuid和自增id做主键哪个好，为什么？.md)
