...
> 与DDL相对的是DML，即Data Manipulation Language，用于操作数据。即包括我们常用的 **INSERT、DELETE、UPDATE** 等。
...
[115、MySQL做索引更新的时候，会锁表吗？](2、相关技术/4、数据库-MySQL/Hollis/115、MySQL做索引更新的时候，会锁表吗？.md)
...
1. 新建一张**临时表**
2. 对**原表加共享MDL锁**，禁止原表的写，**只允许查询**操作
3. 逐行**拷贝原表数据到临时表**，且不进行排序
4. 拷贝完成后**升级原表锁为排他MDL锁**，禁止原表读写
5. 对**临时表rename操作，创建索引，完成DDL操作**
...
1. inplace-no-rebuild ：对二级索引的增删改查、修改变长字段长度（如：varchar）、重命名列名都不需要重建原表
2. inplace-rebuild：修改主键索引、增加删除列、修改字符集、创建全文索引等都需要重建原表。
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507182005883.png)
