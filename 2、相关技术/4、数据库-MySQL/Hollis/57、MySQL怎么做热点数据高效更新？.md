#数据库热点数据处理 #热点数据拆分 #请求合并 #更新操作改成插入操作 

# 典型回答

热点行更新存在的问题：[6、秒杀为什么要不用数据库，而引入Redis？](2、相关技术/24、项目/3、数藏项目/13、项目答疑/6、秒杀为什么要不用数据库，而引入Redis？.md)
...
1. **库存拆分**，把一个大的库存拆分成多个小库存，拆分后，一次扣减动作就可以分散到不同的库、表中进行，降低锁粒度提升并发。
   优点：实现较简单
   缺点：存在碎片问题、库存调控不方便
   ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507150932462.png)
...
具体原理见：
[86、阿里的数据库能抗秒杀的原理](9.1、大厂实践/Hollis/86、阿里的数据库能抗秒杀的原理.md)
...
这个方案的本质上是想将**实时处理的并发操作转变为批量处理**，以减少数据库的压力并提高效率。这种操作通常分为两个步骤：首先是收集和汇总积分，然后是定期更新用户积分表。以下是这两个步骤的示例 SQL 代码：
