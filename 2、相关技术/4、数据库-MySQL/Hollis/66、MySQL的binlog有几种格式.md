...
binlog是MySQL用于**记录数据库中的所有DDL语句和DML语句的一种二进制日志**。它记录了所有对数据库结构和数据的修改操作，如INSERT、UPDATE和DELETE等。binlog主要用来对数据库进行*数据备份、灾难恢复和数据复制等*操作。binlog的格式分为基于语句的格式和基于行的格式。
...
这种格式现在用的都比较少了，因为**它会导致主从同步的数据不一致问题**。

比如说，当我们使用DELETE或者UPDATE的时候，指定了LIMIT，但是并没有使用order by，那么最终这条语句在主库和从库上的执行结果可能是不一样的（即使同一个库上面，你执行多次结果可能也不一样）。
[69、MySQL的limit+order by为什么会数据重复？](2、相关技术/4、数据库-MySQL/Hollis/69、MySQL的limit+order%20by为什么会数据重复？.md)
