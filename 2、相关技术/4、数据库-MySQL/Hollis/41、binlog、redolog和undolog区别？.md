...
|         | 支持引擎            | 内容                                                                                                                               | 主要作用                     | ACID        | 主从          |
| ------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ----------- | ----------- |
| binlog  | innodb、myisam…… | **==逻辑日志==**；不同格式记录不同内容：<br>1. STATEMENT：DDL、DML等**SQL语句**<br>2. ROW：记录**每一行数据是如何修改的**<br>3. MIXED：根据执行的SQL语句自动选择用STATEMENT还是ROW | 数据备份、崩溃恢复和主从复制           | -           | 保证主从服务器的一致性 |
| redolog | innodb（支持事务）    | **==物理日志==**；记录了某**数据页发了什么变化**                                                                                                   | 崩溃恢复/数据持久化（事务的修改会持久化到磁盘） | 持久性         | -           |
| undolog | innodb（支持事务）    | 数据修改前的快照                                                                                                                         | 事务回滚、MVCC                | 一致性、原子性、隔离性 | -           |
