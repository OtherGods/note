...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507201548350.png)
...
所以，当执行计划中出现`Using filesor`t的是时候，就是一个我们可以优化的方向。（但是，并不是说一定要优化！要看是否有必要以及收益是否够大）
...
- 如果要排序的数据量小于 sort_buffer_size，那么**排序就在内存中完成**。
- 如果排序数据量大于sort_buffer_size，则需要**利用磁盘临时文件辅助排序**。
