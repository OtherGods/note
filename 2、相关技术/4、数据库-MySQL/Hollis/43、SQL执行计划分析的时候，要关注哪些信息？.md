...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507141747800.png)
...
<font size=5 color="red">记忆方式：使用唯一/主键索引做常量查询、非唯一索引查询、走索引时范围查询、用了索引列但索引失效的不回表查询、没用索引列查询或用了索引列且失效且需要回表查询</font>
...
- eq_ref：**唯一索引扫描**，只会扫描索引树中的一个匹配行。
	...
	- 当在连接操作中使用了*唯一索引或主键索引*，并且连接条件是基于这些索引的等值条件时，MySQL通常会选择 eq_ref 连接类型，以提高查询性能。
...
- range：**范围扫描**， 只会扫描索引树中的一个范围来查找匹配的行。
	...
	- 使用索引进行性范围查询
- index：**全索引扫描**， 会 *遍历索引树*【不需要回表查询】 来查找匹配的行
	- <font color="blue" size=5>我理解的</font>：
		- <font color="red" size=5>使用了索引列，但索引失效，并且不需要回表查询；</font>
		- 等
	- `explain select c from t2 where b = 's';`、`explain select id from t2 where id in ('1','2'); -- 这中不知道为什么也是，感觉应该是range`
	- 不符合最左前缀匹配的查询
- ALL：全表扫描， 将遍历全表来找到匹配的行。
	- <font color="blue" size=5>我理解的</font>：
		- <font color="red" size=5>使用了索引列，但索引失效，并且需要回表查询；</font>
		- <font color="red" size=5>没使用索引列</font>
		- 等
	- explain select * from t2 where d = "ni";
	- 使用非索引字段查询

需要注意的是，这里的**index表示的是做了索引树扫描，效率并**不高。以上类型**由快到慢：**
...
接着说一个**很重要！** 的字段，但是经常被忽略的字段extra，这个字段描述了 MySQL 在执行查询时所做的一些附加操作。下面是 Extra 可能的取值及其含义：
- Using where：表示 MySQL 将在存储引擎检索行后，再进行条件过滤（使用 WHERE 子句）；<font color="blue" size=5>查询的列未被索引覆盖</font>，where筛选条件<font color="red" size=5>非索引的前导列</font>或者where筛选条件<font color="red" size=5>非索引列</font>。
	- `explain select * from t2 where d = "ni";`   非索引字段查询
	- `explain select d from t2 where b = "ni";`   未索引覆盖，用联合索引的非前导列查询
- Using index：表示 MySQL 使用了**覆盖索引**（也称为索引覆盖）优化，只需要扫描索引，而无需回到数据表中检索行；
	- `explain select b,c from t2 where a = "ni";`  索引覆盖
- Using index condition：表示查询在索引上执行了部分条件过滤。这通常和**索引下推**有关。
	- `explain select d from t2 where a = "ni" and b like "s%";`   使用到索引下推。
- Using where; Using index：<font color="blue" size=5>查询的列被索引覆盖</font>，并 且 where筛选条件是索引列之一，但<font color="red" size=5>不是索引的前导列</font>，或者where<font color="red" size=5>筛选条件是索引列前导列的一个范围</font>
...
- Using temporary：表示 MySQL 创建了**临时表**来存储查询结果。这通常是在排序或分组时发生的；
	- `explain select count(*),b  from t2 group by b;` 
- Using filesort：表示 MySQL 将使用文件排序而不是索引排序，这通常发生在无法使用索引来进行排序（或分组）时；
	- `explain select count(),b  from t2 group by b;`  参考：[37、order by 是怎么实现的？](2、相关技术/4、数据库-MySQL/Hollis/37、order%20by%20是怎么实现的？.md) 、[94、Using filesort 能优化吗，怎么优化？](2、相关技术/4、数据库-MySQL/Hollis/94、Using%20filesort%20能优化吗，怎么优化？.md)
...
首先看key字段有没有值，**有值表示用到了索引树**，但是**具体是怎么用的，还得看type和extra。**
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507142031132.png)
...
### 情况二：
 ...
 `explain select * from t2 where a = 'Hollis';`
 ![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507141831030.png)
表示用到了索引进行查询，并且用到的是idx_abc这个非唯一索引。

### 情况三：

`explain select * from t2 where f = 'f';`
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507142136945.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507142144014.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507142146313.png)
