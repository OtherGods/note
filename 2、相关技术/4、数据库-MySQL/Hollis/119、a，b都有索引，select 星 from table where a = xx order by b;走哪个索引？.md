...
如果 `WHERE a = xx` 的过滤条件的**选择性较高**（即能过滤掉大量行），MySQL 可能优先使用 a 的索引来快速定位满足条件的记录。  

如果 `WHERE a = xx` 的过滤条件的**选择性较低**（即 a = xx 会匹配大量的记录），而 ORDER BY b 对结果的顺序要求较高，MySQL 可能使用 b 的索引来避免排序操作。