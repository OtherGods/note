...
<font color="red" size=5>事务隔离级别的主要目的是<font color="blue">平衡数据的一致性和系统的并发性</font></font>，一般来说更高的隔离级别虽然能提高数据的一致性，但同时也可能降低系统的并发能力。
...
RC与RR对比：[RR 和 RC 的区别](2、相关技术/4、数据库-MySQL/Hollis/17、为什么默认RR，大厂要改成RC？.md#RR%20和%20RC%20的区别)
...
4. **可串行化**(Serializable)是最高的隔离级别，前面提到的所有的隔离级别都无法解决的幻读，在可串行化的隔离级别中可以解决。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202507162115130.png)
...
`set autocommit=off` 或者 `start transaction` 或 `begin`

## 为什么MySQL默认的事务隔离级别是RR？

[16、为什么MySQL默认使用RR隔离级别？](2、相关技术/4、数据库-MySQL/Hollis/16、为什么MySQL默认使用RR隔离级别？.md)
