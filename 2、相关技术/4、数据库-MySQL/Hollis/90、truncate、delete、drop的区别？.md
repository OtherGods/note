...
MySQL数据库中的 `TRUNCATE`、`DELETE`、`DROP` 是用于删除数据的三个不同命令。
1. **`DELETE`** 用于删除**表中的一行或多行记录**，它可以与 WHERE 子句一起使用来指定要删除的记录。
	- 操作过程中*会记录`binlog`*，并且他是在事务中的，**可以做回滚**
	- 自增ID不会重置
	- 删除最慢
1. **`TRUNCATE`** 用于**快速删除表中的所有记录**，并**重置任何自增的计数器（如自增的主键）**
2. **`DROP`** 用于**删除整个表结构及其数据**。
	- 删除表中的**数据**，还删除**表结构**，并且操作**不可撤销**
	- 删除最彻底

这三个操作中，`TRUNCATE` 和 `DROP` 是DDL(数据定义语言)操作，而DELETE是DML（数据操作语言）操作。
