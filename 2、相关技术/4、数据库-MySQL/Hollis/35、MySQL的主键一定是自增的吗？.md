...
如果我们在一张表中没有定义主键，那么，**MySQL会默认选择一个*唯一的非空索引*作为聚簇索引**。

如果**没有适合的非空唯一索引，则会创建一个隐藏的主键（row_id）作为聚簇索引**。
