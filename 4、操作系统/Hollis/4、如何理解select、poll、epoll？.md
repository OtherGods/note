...
select, poll, 和 epoll 都是 Linux 中常见的 I/O 多路复用技术，它们可以用于同时监听多个文件描述符（file descriptor，后文简称fd），当任意一个文件描述符就绪时，就能够**非阻塞的读写数据**。
...
> ChatGPT解释：
> **select、poll都是阻塞的**，但提供了超时参数，可以设置一个超时时间，当监视的文件描述符中没有任何一个准备好执行IO操作时，select函数会在超时时间到达后返回，这样也可以避免select函数永久阻塞。因此，虽然select函数在执行时是阻塞的，但是通过设置超时时间或者使用多线程等方式，可以使得IO多路复用仍然是同步非阻塞的。
...
函数签名如下：
```C
// select方法的参数timeout可以设置超时时间，防止因为select是阻塞的而导致程序一直阻塞
