...
[1、介绍一下OSI七层模型？](5、计算机网络/Hollis/1、介绍一下OSI七层模型？.md)

他们主要有以下区别：
1. 连接类型
	- TCP 是一种面向连接的协议。在发送数据之前，它需要 **==建立连接==**，这通过三次握手过程完成。结束后通过四次挥手断开连接。
	- UDP 是无连接的协议。它发送数据而 **==不预先建立连接==**。
2. 可靠性
	- TCP 提供可靠的数据传输，通过 **==确认和重传机制==** 来确保数据的正确送达。
...
[介绍下TCP是如何实现拥塞控制的？](5、计算机网络/Hollis/介绍下TCP是如何实现拥塞控制的？.md)
...
| 可靠性    | 可靠的数据传输，有**确认重传**机制，若没有确认，数据会重新被发送 | 不可靠，**不保证发送的数据是否成功被接收**           |
| 头部大小   | 20-60字节长度                          | 固定8字节                             |
| 速度     | 比UDP**慢**                          | 比TCP**快**                         |
| 顺序性    | 所有数据包按**特定顺序**传递给接收者               | 数据报以**无顺序**送达                     |
| 错误检测机制 | 提供了**流量控制、拥塞控制（慢启动、快重传、快恢复）**      | 只有基本的错误检测机制，如校验和                  |
