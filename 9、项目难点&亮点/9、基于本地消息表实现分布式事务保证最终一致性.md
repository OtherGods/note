...
1. **消息发送失败，MQ没接到消息**：这种情况，messageService.send(orderDTO);会抛异常，那么本地事务捕获到这个异常之后，把createOrder回滚了就行了
2. **MQ接到了消息，但是客户端因为网络延迟以为失败了**：这种情况比较复杂了，就是说客户端因为接到了失败的response，会直接回滚createOrder。但是MQ收到了消息之后，会投递给积分系统，积分系统会直接消费消息，然后增加积分
...
[7、什么是分布式事务？](2、相关技术/21、分布式/Hollis/7、什么是分布式事务？.md)

[8、常见的分布式事务有哪些？](2、相关技术/21、分布式/Hollis/8、常见的分布式事务有哪些？.md)
