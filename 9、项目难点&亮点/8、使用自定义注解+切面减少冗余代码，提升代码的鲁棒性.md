相信很多人对Java中的注解都很熟悉，比如我们经常会用到的一些如`@Override`、`@Autowired`、`@Service`等，这些都是JDK或者诸如Spring这类框架给我们提供的。
...
2. 使用`Spel`的相关方法，通过指定的标识，**从对应的参数中获取到目标对象的唯一性标识**。 
3. 在方法执行成功后，输出日志。
...
这种是入参的参数列表中已经有了被操作的对象的唯一性标识，直接使用`#id`指定即可。
...
以上，就是一个简单的使用 **自定义注解 + 切面** 进行日志记录的场景。下面我们再来看一个如何使用注解做方法参数的校验。
...
以上，对`userName`参数注明不能为null。

然后再使用`hibernate validator`定义一个工具类，用于做参数校验。
```java
/**
 * 参数校验工具
 *
...
总之，使用 **切面 + 自定义注解**，我们可以统一做很多事情。除了以上的这几个场景，我们还有很多相似的用法，比如：
