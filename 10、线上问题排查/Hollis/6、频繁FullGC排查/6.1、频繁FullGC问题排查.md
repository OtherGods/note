...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508030947998.png)
...
3. 该**类对应的`java.lang.Class`对象没有任何地方被引用**。
[23、Java中的类什么时候会被加载？](2、相关技术/3、JVM/Hollis/23、Java中的类什么时候会被加载？.md)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508030950966.png)

![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508030951336.png)

可以看到，存在非常多的`Scrip_${timestamp}_${idx}`类型的类
...
直接看编码生成器的初始化阶段
- 首先，获取`AviatorClassLoader`类型的类加载器
- 初始化`OptimizeCodeGenerator`的编码生成器
	- 类型为`ASMCodeGenerator`
	- `classLoader=AviatorClassLoader`
	- `className="Script_" + System.currentTimeMillis() + "" + CLASS_COUNTER.getAndIncrement();`
	- 与metaspace区的类信息吻合
```java
// 初始化编码生成器
...
结论就是，**无缓存情况下，每次编译表达式都会生成一个`AviatorClassLoader类加载器+一个匿名的Express类型的class`，metaspace的空间会逐步占满**
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508031007458.png)

# 问题解决

解决思路就是官方支持使用缓存的方式。在github上也有人提到过类似的问题。
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508031007879.png)

- 使用cacheKey的缓存方案
	- `public Expression compile(final String cacheKey, final String expression, final boolean cached)`
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508031008892.png)

- key组装规则
	- `${product}_${priceBaseDTO.code}{simpleName}_{method}`
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202508031008231.png)
