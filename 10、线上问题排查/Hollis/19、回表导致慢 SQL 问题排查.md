...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202510091551557.png)
...
![image.png](https://raw.githubusercontent.com/OtherGods/MaterialImage/main/img/202510091552069.png)

这个用户确实是一个大买家、表中一共100多万条数据，他自己就有几十万单（新的业务模式，之前也没见过）

并且在分析过程中，发现这个 SQL 有的时候走索引，有的时候不走索引，并且走不走索引都很慢。于是看一下成本消耗到底在哪里。
[109、MySQL的优化器的索引成本是怎么算出来的？](2、相关技术/4、数据库-MySQL/Hollis/109、MySQL的优化器的索引成本是怎么算出来的？.md)
```sql
EXPLAIN FORMAT=json
